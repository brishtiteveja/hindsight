...
import { D as n, H as r, K as i, U as a, a as o, b as s, c, f as l, v as u, w as d, y as f, z as p } from "./chunk-DU6HZSFF-Bv553b-w.mjs";
import { t as m } from "./linear-DhhH3tye.mjs";
import { t as h } from "./ordinal-B1BNQwUR.mjs";
import { t as g } from "./init-CsQUQz36.mjs";
import { i as _ } from "./chunk-75Z2AOVW-DlpwHa_5.mjs";
import { t as v } from "./line-CuXwheRf.mjs";
import "./chunk-PWAF6VOD-CPL-Te_E.mjs";
import { t as y } from "./chunk-GMAD6QVW-4mlTi1Bn.mjs";
import { f as b } from "./mermaid.core-DuekSTCI.mjs";
...
var C, w, T, E, D, O, k, A, j, M, N, P = (function() {
	...
	return e(j, "Parser"), j.prototype = A, A.Parser = j, new j();
})();
P.parser = P;
var ee = P;
function F(e) {
...
var R = (C = class {
	...
	getMaxDimension(e, t) {
		...
		for (let i of e) {
			let e = y(r, 1, i), a = e ? e.width : i.length * t, o = e ? e.height : t;
			n.width = Math.max(n.width, a), n.height = Math.max(n.height, o);
		}
		return r.remove(), n;
	}
}, e(C, "TextDimensionCalculatorWithFont"), C), te = .7, ne = .2, re = (w = class {
	constructor(e, t, n, r) {
	...
	calculateSpaceIfDrawnHorizontally(e) {
		...
		if (this.axisConfig.showAxisLine && t > this.axisConfig.axisLineWidth && (t -= this.axisConfig.axisLineWidth, this.showAxisLine = !0), this.axisConfig.showLabel) {
			...
			let i = n.height;
			this.axisPosition === "bottom" && this.normalizedLabelRotationInRad !== 0 && (i = Math.max(i, Math.abs(Math.sin(this.normalizedLabelRotationInRad) * n.width) + Math.abs(Math.cos(this.normalizedLabelRotationInRad) * n.height))), i += this.axisConfig.labelPadding * 2, this.labelTextHeight = n.height, i <= t && (t -= i, this.showLabel = !0);
		}
	...
	calculateOffsetByRotation(e) {
		let t = this.normalizedLabelRotationInRad;
		return t === 0 ? 0 : Math.sin(t) * this.getLabelDimension()[e] / 2;
	}
...
}, e(w, "BaseAxis"), w), ie = (T = class extends re {
	...
	recalculateScale() {
		this.scale = S().domain(this.categories).range(this.getRange()).paddingInner(1).paddingOuter(0).align(.5), t.trace("BandAxis axis final categories, range: ", this.categories, this.getRange());
	}
	getTickValues() {
...
}, e(T, "BandAxis"), T), ae = (E = class extends re {
	constructor(e, t, n, r, i) {
...
function z(e, t, n, r) {
	let i = new R(r);
	return I(e) ? new ie(t, n, e.categories, e.title, i) : new ae(t, n, [e.min, e.max], e.title, i);
}
e(z, "getAxis");
var oe = (D = class {
	constructor(e, t, n, r) {
...
function se(e, t, n, r) {
	return new oe(new R(r), e, t, n);
}
e(se, "getChartTitleComponent");
var ce = .75, le = .5, ue = .35;
function B(e) {
	return {
		fontSize: e,
		markerSize: e * ce,
...
var de = (O = class {
	...
	calculateSpace(e) {
		...
		let { fontSize: t, markerSize: n, markerSpacing: r, itemSpacing: i } = B(this.chartConfig.legendFontSize), a = this.textDimensionCalculator.getMaxDimension(this.visiblePlots.map((e) => e.title), t), o = this.chartConfig.legendPadding * 2 + n + r + a.width, s = this.chartConfig.legendPadding * 2 + this.visiblePlots.length * t + (this.visiblePlots.length - 1) * i;
		return o <= e.width && s <= e.height ? (this.boundingRect.width = o, this.boundingRect.height = s) : (this.visiblePlots = [], this.boundingRect.width = 0, this.boundingRect.height = 0), {
			width: this.boundingRect.width,
			height: this.boundingRect.height
		};
	}
	getDrawableElements() {
		...
		let { fontSize: e, markerSize: t, markerSpacing: n, itemSpacing: r } = B(this.chartConfig.legendFontSize), i = e + r, a = this.boundingRect.x + this.chartConfig.legendPadding, o = this.boundingRect.y + this.chartConfig.legendPadding, s = [], c = [];
		for (let [e, n] of this.visiblePlots.entries()) if (F(n)) s.push({
			x: a,
			y: o + e * i,
			width: t,
			height: t,
		...
		return [
			...
			},
			{
				...
				type: "text",
				data: this.visiblePlots.map((r, s) => ({
					text: r.title,
					x: a + t + n,
					y: o + s * i + t / 2,
...
function fe(e, t, n, r) {
	return new de(new R(r), e, t, n);
}
e(fe, "getChartLegendComponent");
var pe = (k = class {
	constructor(e, t, n, r, i) {
	...
	getDrawableElement() {
		...
		if (t = this.orientation === "horizontal" ? v().y((e) => e[0]).x((e) => e[1])(e) : v().x((e) => e[0]).y((e) => e[1])(e), !t) return [];
		let n = [{
		...
		if (this.plotData.pointLabels && this.plotData.pointLabels.length > 0) {
			...
			for (let [n, [r, i]] of e.entries()) {
				let e = this.plotData.pointLabels[n];
				e && (this.orientation === "horizontal" ? t.push({
					x: i + 10,
					y: r,
					text: e,
	...
	getDrawableElements() {
		...
		for (let [t, n] of this.chartData.plots.entries()) switch (n.type) {
			case "line":
				{
					let r = new pe(n, this.xAxis, this.yAxis, this.chartConfig.chartOrientation, t);
					e.push(...r.getDrawableElement());
				}
				break;
			case "bar": {
				let r = new me(n, this.boundingRect, this.xAxis, this.yAxis, this.chartConfig.chartOrientation, t);
				e.push(...r.getDrawableElement());
			}
		}
		return e;
...
var _e = (M = class {
	...
	calculateVerticalSpace() {
		let e = this.chartConfig.width, t = this.chartConfig.height, n = 0, r = 0, i = {
			width: 0,
			height: 0
		}, a = Math.floor(e * this.chartConfig.plotReservedSpacePercent / 100), o = Math.floor(t * this.chartConfig.plotReservedSpacePercent / 100), s = this.componentStore.plot.calculateSpace({
			width: a,
			height: o
		});
		e -= s.width, t -= s.height, s = this.componentStore.title.calculateSpace({
			...
			height: t
		}), r = s.height, t -= s.height, this.componentStore.xAxis.setAxisPosition("bottom"), s = this.componentStore.xAxis.calculateSpace({
			width: e,
			height: t
		}), t -= s.height, this.componentStore.yAxis.setAxisPosition("left"), s = this.componentStore.yAxis.calculateSpace({
			width: e,
			height: t
		}), n = s.width, e -= s.width, i = this.componentStore.legend.calculateSpace({
			width: e,
			height: o
		}), e -= i.width, e > 0 && (a += e, e = 0), t > 0 && (o += t, t = 0), this.componentStore.plot.calculateSpace({
			width: a,
			height: o
			...
			x: n + a,
			y: r + Math.max((o - i.height) / 2, 0)
		}), this.componentStore.xAxis.setRange([n, n + a]), this.componentStore.xAxis.setBoundingBoxXY({
			x: n,
			y: r + o
		}), this.componentStore.yAxis.setRange([r, r + o]), this.componentStore.yAxis.setBoundingBoxXY({
			x: 0,
			y: r
		}), this.chartData.plots.some((e) => F(e)) && this.componentStore.xAxis.recalculateOuterPaddingToDrawBar();
	}
	calculateHorizontalSpace() {
		let e = this.chartConfig.width, t = this.chartConfig.height, n = 0, r = 0, i = 0, a = {
			width: 0,
			height: 0
		}, o = Math.floor(e * this.chartConfig.plotReservedSpacePercent / 100), s = Math.floor(t * this.chartConfig.plotReservedSpacePercent / 100), c = this.componentStore.plot.calculateSpace({
			width: o,
			height: s
		});
		e -= c.width, t -= c.height, c = this.componentStore.title.calculateSpace({
			width: this.chartConfig.width,
			height: t
		}), n = c.height, t -= c.height, this.componentStore.xAxis.setAxisPosition("left"), c = this.componentStore.xAxis.calculateSpace({
			width: e,
			height: t
		}), e -= c.width, r = c.width, this.componentStore.yAxis.setAxisPosition("top"), c = this.componentStore.yAxis.calculateSpace({
			width: e,
			height: t
		}), t -= c.height, i = n + c.height, a = this.componentStore.legend.calculateSpace({
			width: e,
			height: s
		}), e -= a.width, e > 0 && (o += e, e = 0), t > 0 && (s += t, t = 0), this.componentStore.plot.calculateSpace({
			width: o,
			height: s
		}), this.componentStore.plot.setBoundingBoxXY({
			x: r,
			...
			x: r + o,
			y: i + Math.max((s - a.height) / 2, 0)
		}), this.componentStore.yAxis.setRange([r, r + o]), this.componentStore.yAxis.setBoundingBoxXY({
			x: r,
			y: n
		}), this.componentStore.xAxis.setRange([i, i + s]), this.componentStore.xAxis.setBoundingBoxXY({
			x: 0,
			y: i
		}), this.chartData.plots.some((e) => F(e)) && this.componentStore.xAxis.recalculateOuterPaddingToDrawBar();
	}
...
}, e(M, "Orchestrator"), M), ve = (N = class {
	static build(e, t, n, r) {
		return new _e(e, t, n, r).getDrawableElement();
	}
}, e(N, "XYChartBuilder"), N), V = 0, ye, H = Y(), U = J(), W = X(), G = U.plotColorPalette.split(",").map((e) => e.trim()), K = !1, q = !1;
function J() {
	let e = n(), t = s();
	return _(e.xyChart, t.themeVariables.xyChart);
}
e(J, "getChartDefaultThemeConfig");
function Y() {
	let e = s();
	return _(l.xyChart, e.xyChart);
}
e(Y, "getChartDefaultConfig");
function X() {
	return {
		yAxis: {
			type: "linear",
...
e(Q, "setXAxisRangeData");
function Ce(e) {
	W.xAxis = {
		type: "band",
		title: W.xAxis.title,
		categories: e.map((e) => Z(e.text))
	}, K = !0;
}
e(Ce, "setXAxisBand");
function we(e) {
	W.yAxis.title = Z(e.text);
}
e(we, "setYAxisTitle");
function Te(e, t) {
	W.yAxis = {
...
e(Te, "setYAxisRangeData");
function Ee(e) {
	let t = Math.min(...e), n = Math.max(...e), r = L(W.yAxis) ? W.yAxis.min : Infinity, i = L(W.yAxis) ? W.yAxis.max : -Infinity;
	W.yAxis = {
		type: "linear",
		title: W.yAxis.title,
		min: Math.min(r, t),
...
e(Ee, "setYAxisRangeFromPlotData");
function $(e) {
	let t = [];
	if (e.length === 0) return t;
	if (!K) {
		let t = L(W.xAxis) ? W.xAxis.min : Infinity, n = L(W.xAxis) ? W.xAxis.max : -Infinity;
		Q(Math.min(t, 1), Math.max(n, e.length));
	}
	if (I(W.xAxis) && e.length > W.xAxis.categories.length && (e = e.slice(0, W.xAxis.categories.length)), q || Ee(e), I(W.xAxis) && (t = W.xAxis.categories.map((t, n) => [t, e[n]])), L(W.xAxis)) {
		let n = W.xAxis.min, r = W.xAxis.max;
		if (e.length === 1) t = [[`${n}`, e[0]]];
		else {
			let i = (r - n) / (e.length - 1);
			t = e.map((e, t) => [`${n + t * i}`, e]);
		}
	}
	return t;
}
e($, "transformDataWithoutCategory");
function De(e) {
	return G[e === 0 ? 0 : e % G.length];
}
e(De, "getPlotColorFromPalette");
function Oe(e, t) {
	let n = t.map((e) => e.value), r = t.map((e) => e.label ? Z(e.label) : ""), i = $(n), a = r.some((e) => e !== "");
	W.plots.push({
		type: "line",
		title: Z(e.text),
		strokeFill: De(V),
...
e(ke, "setBarData");
function Ae() {
	if (W.plots.length === 0) throw Error("No Plot to render, please provide a plot with some data");
	return W.title = d(), ve.build(H, W, U, ye);
}
e(Ae, "getDrawableElem");
function je() {
	return U;
}
e(je, "getChartThemeConfig");
function Me() {
	return H;
}
e(Me, "getChartConfig");
...
var Pe = {
	...
	db: {
		...
		clear: /* @__PURE__ */ e(function() {
			o(), V = 0, H = Y(), W = X(), U = J(), G = U.plotColorPalette.split(",").map((e) => e.trim()), K = !1, q = !1;
		}, "clear"),
		setAccTitle: a,
		getAccTitle: f,
		setDiagramTitle: i,
		...
		setXAxisBand: Ce,
		setYAxisTitle: we,
		setYAxisRangeData: Te,
		setLineData: Oe,
		setBarData: ke,
		setTmpSVGG: be,
		getChartThemeConfig: je,
		getChartConfig: Me,
		getXYChartData: Ne
	},
	renderer: { draw: /* @__PURE__ */ e((n, r, i, a) => {
		let o = a.db, s = o.getChartThemeConfig(), l = o.getChartConfig(), u = o.getXYChartData().plots[0].data.map((e) => e[1]);
		function d(e) {
			return e === "top" ? "text-before-edge" : "middle";
		}
		e(d, "getDominantBaseLine");
		function f(e) {
		...
		e(p, "getTextTransformation"), t.debug("Rendering xychart chart\n" + n);
		let m = b(r), h = m.append("g").attr("class", "main"), g = h.append("rect").attr("width", l.width).attr("height", l.height).attr("class", "background");
		c(m, l.height, l.width, !0), m.attr("viewBox", `0 0 ${l.width} ${l.height}`), g.attr("fill", s.backgroundColor), o.setTmpSVGG(m.append("g").attr("class", "mermaid-tmp-group"));
		...
		for (let t of _) {
			...
			let n = y(t.groupTexts);
			switch (t.type) {
				case "rect":
					if (n.selectAll("rect").data(t.data).enter().append("rect").attr("x", (e) => e.x).attr("y", (e) => e.y).attr("width", (e) => e.width).attr("height", (e) => e.height).attr("fill", (e) => e.fill).attr("stroke", (e) => e.strokeFill).attr("stroke-width", (e) => e.strokeWidth), l.showDataLabel) {
						let r = l.showDataLabelOutsideBar;
						if (l.chartOrientation === "horizontal") {
							let i = function(e, t) {
								let { data: n, label: r } = e;
								return t * r.length * a <= n.width - o;
							};
							e(i, "fitsHorizontally");
							let a = .7, o = 10, c = t.data.map((e, t) => ({
								data: e,
								label: u[t].toString()
								...
								for (; !i(e, n) && n > 0;) --n;
								return n;
							}), d = Math.floor(Math.min(...l)), f = /* @__PURE__ */ e((e) => r ? e.data.x + e.data.width + 10 : e.data.x + e.data.width - 10, "determineLabelXPosition");
							...
							let a = t.data.map((e, t) => ({
								...
								for (; !i(e, r, 10) && r > 0;) --r;
								return r;
							}), c = Math.floor(Math.min(...o)), l = /* @__PURE__ */ e((e) => r ? e.data.y - 10 : e.data.y + 10, "determineLabelYPosition");
					...
					break;
				case "text":
					n.selectAll("text").data(t.data).enter().append("text").attr("x", 0).attr("y", 0).attr("fill", (e) => e.fill).attr("font-size", (e) => e.fontSize).attr("dominant-baseline", (e) => d(e.verticalPos)).attr("text-anchor", (e) => f(e.horizontalPos)).attr("transform", (e) => p(e)).text((e) => e.text);
					break;
				case "path": n.selectAll("path").data(t.data).enter().append("path").attr("d", (e) => e.path).attr("fill", (e) => e.fill ? e.fill : "none").attr("stroke", (e) => e.strokeFill).attr("stroke-width", (e) => e.strokeWidth);
			}
		}
	}, "draw") }
};
