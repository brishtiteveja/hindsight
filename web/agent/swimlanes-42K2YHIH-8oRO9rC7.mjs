...
import { o as n } from "./chunk-OSK3NFVY-Bbxjh8pF.mjs";
import "./chunk-L3NEJ4N5-xQTuRmpd.mjs";
import { a as r } from "./mermaid.core-DuekSTCI.mjs";
//#region node_modules/mermaid/dist/chunks/mermaid.core/swimlanes-42K2YHIH.mjs
var i = 5, a = 1e-5, o = 1e-6;
function s(e) {
	let t = [];
...
function h(e, t) {
	...
	let r = e.map((e) => ({ ...e })), i = t.arrowTypeStart && n[t.arrowTypeStart];
	if (i) {
		let t = e[0], n = e[1], a = Math.atan2(n.y - t.y, n.x - t.x);
		r[0].x = t.x + i * Math.cos(a), r[0].y = t.y + i * Math.sin(a);
	}
	let a = t.arrowTypeEnd && n[t.arrowTypeEnd];
	if (a) {
...
function g(e, t, n, r, i) {
	...
	return i === "arc" ? l.push(`A${d(e.r)},${d(e.r)} 0 0 ${r} ${f(c)}`) : l.push(`M${f(c)}`), l;
}
e(g, "emitJump");
function _(e, t, n, r) {
	let i = t.x - e.x, o = t.y - e.y, s = n.x - t.x, c = n.y - t.y, l = Math.hypot(i, o), u = Math.hypot(s, c);
	if (l < a || u < a) return null;
	let d = i / l, f = o / l, p = s / u, m = c / u, h = d * p + f * m, g = Math.acos(Math.max(-1, Math.min(1, h)));
	if (g < a || Math.abs(Math.PI - g) < a) return null;
	let _ = Math.min(r / Math.sin(g / 2), l / 2, u / 2);
	return {
		startX: t.x - d * _,
		startY: t.y - f * _,
		endX: t.x + p * _,
...
e(_, "computeRoundedCorner");
function v(e, t, n) {
	let r = e.points;
	if (r.length < 2) return "";
	let a = h(r, e), o = e.curve === "rounded", c = s(a), l = /* @__PURE__ */ new Map();
	for (let e of t) {
		let t = c[e.segIndex];
		if (!t) continue;
	...
	for (let e = 0; e < c.length; e++) {
		let t = c[e], r = Math.hypot(t.b.x - t.a.x, t.b.y - t.a.y), s = r === 0 ? 0 : (t.b.x - t.a.x) / r, h = r === 0 ? 0 : (t.b.y - t.a.y) / r, v = p(t), y = 0;
		if (o && e > 0) {
			let t = _(a[e - 1], a[e], a[e + 1] ?? a[e], i);
			t && (y = t.cutLen);
		}
		let b = r, x = null;
		o && e < c.length - 1 && (x = _(a[e], a[e + 1], a[e + 2] ?? a[e + 1], i), x && (b = r - x.cutLen));
		...
		for (let e of S) e.r < m || u.push(...g(e, s, h, v, n.jumpStyle));
		o && x ? (u.push(`L${d(x.startX)},${d(x.startY)}`), u.push(`Q${d(x.ctrlX)},${d(x.ctrlY)} ${d(x.endX)},${d(x.endY)}`)) : u.push(`L${f(t.b)}`);
	}
	return u.join(" ");
}
e(v, "rewriteEdgePath");
...
function S(e, t, n) {
	...
	for (let e of t) {
		...
		if (!n) continue;
		o.set(e.id, n);
		let i = x(n.getAttribute("data-points")) ?? e.points;
		a.push({
			...e,
			points: i
		});
	...
	for (let e of a) {
		...
		if (r !== void 0 && !b(r)) continue;
		let a = o.get(e.id);
		if (!a || r === void 0 && !y(a.getAttribute("d") ?? "")) continue;
		let s = a.getAttribute("style") ?? "", l = /stroke-dasharray\s*:\s*0\s+([\d.]+)\s+[\d.]+\s+([\d.]+)/.exec(s), u = l ? Number.parseFloat(l[1]) : null, d = l ? Number.parseFloat(l[2]) : null, f = v(e, t, n);
...
e(S, "applyLineJumpsToSvg");
function C(e, { measure: t }) {
	...
	S(t.groups.edgePaths, i, {
		enabled: !0,
		jumpRadius: 6,
		jumpStyle: r
	});
}
e(C, "applySwimlaneLineJumps");
var w = "__swimlane_default__", T = 21, E = 20;
function D(e) {
	return Math.max(e.padding ?? E, E);
}
e(D, "topLaneHorizontalPadding");
function O(e) {
	...
	let o = n - i / 2, s = Math.min(a, n + i / 2), c = o + Math.min(T, Math.max(0, s - o));
	if (c <= o) {
		delete e.groupTitleRect;
		return;
	}
...
function k(e) {
	...
	for (let e of r) e.parentId = w;
}
e(k, "prepareLayoutForSwimlanes");
function A(e) {
...
function j(e, t, n, r) {
	...
	for (let e of l) {
		...
		if (r === Infinity || a === Infinity) e.x = e.x ?? 0, e.y = e.y ?? 0, e.width = e.width ?? 0, e.height = e.height ?? 0;
			...
			let t = e.padding ?? 20, n = e.parentId ? t : 2 * D(e), s = t, c = Math.max(0, i - r) + n, l = Math.max(0, o - a) + s, d = (r + i) / 2, f = (a + o) / 2;
	...
	if (d.length > 0 && u.size > 0) {
		...
		if (e !== Infinity && t !== -Infinity) {
			...
			for (let e of a) {
				let t = u.get(e.id);
				if (!t) continue;
				let n = Math.max(0, t.maxX - t.minX) + 2 * D(e), r = (t.minX + t.maxX) / 2;
			...
			if (l > 0) {
				...
				for (let t of d) {
					let n = e.get(t.id);
					n != null && (t.width = n), O(t);
				}
			}
		}
	}
}
e(j, "writeBackToLayoutData");
var ee = "[EdgeLabelNodes]";
function te(e) {
	...
	for (let a of e.edges) {
		...
		if (!e || !o) {
			t.warn(ee, `Edge ${a.id} has missing source or target node`);
			continue;
		}
...
e(I, "sameY");
function L(e, t, n = M) {
	return I(e, t, n) && Math.abs(e.x - t.x) > n;
}
e(L, "isHorizontalSegment");
function R(e, t, n = M) {
	return F(e, t, n) && Math.abs(e.y - t.y) > n;
}
e(R, "isVerticalSegment");
function z(e, t, n, r) {
	return Math.max(0, Math.min(Math.max(e, t), Math.max(n, r)) - Math.max(Math.min(e, t), Math.min(n, r)));
}
e(z, "overlapLength");
function B(e, t, n = M) {
	return e.horizontal && t.horizontal && I(e.a, t.a, n) ? z(e.a.x, e.b.x, t.a.x, t.b.x) : e.vertical && t.vertical && F(e.a, t.a, n) ? z(e.a.y, e.b.y, t.a.y, t.b.y) : 0;
}
e(B, "sameAxisSegmentOverlapLength");
function re(e, t = M) {
	let n = [];
	for (let r = 0; r < e.length - 1; r++) {
		let i = e[r], a = e[r + 1], o = L(i, a, t), s = R(i, a, t);
		(o || s) && n.push({
			index: r,
			a: i,
			b: a,
...
e(H, "dedupeConsecutivePoints");
function ie(e, t = M) {
	if (!e || e.length !== 4) return;
	let [n, r, i, a] = e;
	return L(n, r, t) && R(r, i, t) && L(i, a, t) ? {
		kind: "HVH",
		p0: n,
		p1: r,
		p2: i,
		p3: a
	} : R(n, r, t) && L(r, i, t) && R(i, a, t) ? {
		kind: "VHV",
		p0: n,
		p1: r,
		p2: i,
...
e(ie, "classifyThreeSegmentRoute");
...
e(se, "rectFromCenterSize");
function K(e) {
	return ne(e)?.rect;
}
e(K, "rectOfNodeBounds");
function ce(e, t) {
	switch (t) {
		case "top": return {
			x: e.cx,
...
e(q, "collectNodeRectEntries");
function fe(e, { includeEdgeLabels: t = !0 } = {}) {
	let n = [];
	for (let r of e) {
...
e(pe, "getNodePairGeometry");
function J(e, t, n, r = [], i = 0) {
	for (let a of n) if (!r.includes(a.id) && ae(e, t, a.rect, -i)) return !0;
	return !1;
}
e(J, "segmentHitsAnyRect");
function me(e, t, n, r, i = M, a = 1e-6) {
	let o = I(e, t, i), s = F(e, t, i), c = I(n, r, i), l = F(n, r, i);
...
function he(e, t, n, r, i = M) {
	let a = I(e, t, i), o = F(e, t, i), s = I(n, r, i), c = F(n, r, i);
	return o && c && F(e, n, i) ? z(e.y, t.y, n.y, r.y) > i : a && s && I(e, n, i) ? z(e.x, t.x, n.x, r.x) > i : !1;
}
e(he, "sameAxisSegmentsOverlap");
function ge(e, t, n, r, { epsilon: i = M, skipDegenerateOther: a = !1 } = {}) {
	for (let o of n) {
		if (o === r || o.isLayoutOnly) continue;
		let n = o.points;
		if (!(!n || n.length < 2)) for (let r = 0; r < n.length - 1; r++) {
			let o = n[r], s = n[r + 1];
			if (!(a && P(o, s, i)) && (me(e, t, o, s, i) || he(e, t, o, s, i))) return !0;
		}
	}
	return !1;
}
e(ge, "segmentConflictsWithAnyEdge");
function _e(e, t, n, r, i = M) {
	let a = I(e, t, i), o = F(e, t, i), s = I(n, r, i), c = F(n, r, i);
	if (!(a && c || o && s)) return !1;
	let l = a ? {
		a: e,
...
e(_e, "orthogonalSegmentsStrictlyCross");
function ve(e, t, n) {
	let r = Math.min(t, n), i = Math.max(t, n);
	return e > r + M && e < i - M;
}
e(ve, "strictlyBetween");
function ye(e, t, n) {
	return F(e, t) && F(t, n) ? ve(t.y, e.y, n.y) : I(e, t) && I(t, n) ? ve(t.x, e.x, n.x) : !1;
}
e(ye, "isCollinearIntermediate");
function be(e) {
	let t = !1, n = [];
...
e(be, "simplifyPolylineOnce");
function xe(e) {
	let t = [e[0]];
	for (let n = 1; n < e.length; n++) {
		let r = t[t.length - 1], i = e[n];
		if (!F(r, i) && !I(r, i)) {
			let e = t.length >= 2 ? t[t.length - 2] : void 0, n = e && F(e, r) ? {
				x: r.x,
				y: i.y
			} : {
				x: i.x,
...
e(Ee, "segmentEnterPoint");
function De(e, t, n) {
	let r = n ? 1 : -1, i = n ? 0 : e.length - 1;
	for (; i >= 0 && i < e.length && U(e[i], t, Ce);) i += r;
...
function Oe(e, t) {
	for (let n of e) {
		...
		if (!e) continue;
		let r = [...e.points];
		e.srcRect && (r = De(r, e.srcRect, !0)), e.dstRect && (r = De(r, e.dstRect, !1)), r = Se(xe(r)), r = ze(r, e.srcRect, e.dstRect), e.edge.points = Se(xe(r));
	}
}
e(Oe, "clipEdgeEndpointsToNodeBoundaries");
...
e(je, "cornerClearanceRange");
function Me(e, t, n) {
	let { lo: r, hi: i } = je(t, n);
	return Math.min(i, Math.max(r, e));
}
e(Me, "clampToCornerClearance");
function Ne(e) {
...
e(Ne, "intersectRanges");
function Pe(e, t) {
	return t === "left" || t === "right" ? je(e.top, e.bottom) : je(e.left, e.right);
}
e(Pe, "clearanceRangeForSide");
function Fe(e, t, n) {
...
e(Fe, "terminalSideForSegment");
function Ie(e) {
	return e === "left" || e === "right";
}
e(Ie, "isHorizontalSide");
function Le(e, t, n, r, i) {
	let a = [], o = n ? Fe(e, t, n) : void 0, s = r ? Fe(t, e, r) : void 0;
	return n && o && Ie(o) === i && a.push(Pe(n, o)), r && s && Ie(s) === i && a.push(Pe(r, s)), a.length > 0 ? Ne(a) : void 0;
}
e(Le, "straightClearanceRange");
...
e(Re, "clearStraightEndpointCornerAxis");
function ze(e, t, n) {
	if (e.length !== 2) return e;
	let [r, i] = e;
	return I(r, i, Y) ? Re(r, i, t, n, !0) ?? e : F(r, i, Y) ? Re(r, i, t, n, !1) ?? e : e;
}
e(ze, "clearStraightEndpointCornerConnections");
function Be(e, t, n) {
	return Ie(n) ? {
		x: e.x,
		y: Me(e.y, t.top, t.bottom)
	} : {
		x: Me(e.x, t.left, t.right),
		y: e.y
	};
}
e(Be, "cornerClearedEndpoint");
function Ve(e, t, n, r, i, a) {
	let o = e.map((e) => ({ ...e }));
	for (let s = t; s >= 0 && s < e.length; s += n) {
		let t = e[s];
		if (a && !I(t, r, Y) || !a && !F(t, r, Y)) break;
		a ? o[s].y = i.y : o[s].x = i.x;
	}
	return o;
}
e(Ve, "moveCollinearEndpointRun");
function He(e, t, n) {
	if (e.length < 2) return e;
	let r = n ? 0 : e.length - 1, i = n ? 1 : -1, a = e[r], o = Ae(e, r, i);
	if (!o) return e;
	let s = Fe(a, o, t);
	if (!s) return e;
	let c = Ie(s), l = Be(a, t, s);
	return P(a, l, Y) ? e : Ve(e, r, i, a, l, c);
}
e(He, "clearEndpointCornerConnection");
function Ue(e, t, n) {
...
e(Ue, "borderSideForSegment");
function We(e, t, n, r) {
	switch (e) {
		case "top": return F(t, n, Y) && n.y < r.top - Y;
		case "bottom": return F(t, n, Y) && n.y > r.bottom + Y;
		case "left": return I(t, n, Y) && n.x < r.left - Y;
		case "right": return I(t, n, Y) && n.x > r.right + Y;
	}
}
e(We, "leavesOutward");
function Ge(e, t, n) {
	...
	if (n) {
		let n = Ue(e[0], e[1], t);
		return n && We(n, e[1], e[2], t) ? e.slice(1) : e;
	}
	let r = e.length - 1, i = Ue(e[r - 1], e[r], t);
	return i && We(i, e[r - 1], e[r - 2], t) ? e.slice(0, r) : e;
}
e(Ge, "collapseOwnBorderStub");
function Ke(e, t, n) {
	let r = e;
	if (t) {
		let e = Ae(r, 0, 1);
	...
	let i = ze(r, t, n);
	return i !== r || r.length === 2 ? i : (t && (r = He(r, t, !0)), n && (r = He(r, n, !1)), r);
}
e(Ke, "snapAndCollapseEndpoints");
function qe(e, t) {
	for (let n of e) {
		let e = Te(n, t, 2);
		if (!e) continue;
		let r = Ke(H(e.points, Y), e.srcRect, e.dstRect);
		if (r.length < 3) {
			e.edge.points = r;
			continue;
		}
...
e(qe, "prepareEdgeEndpointsForRenderer");
function Je(e) {
...
e(Ze, "boundsForChildren");
function Qe(e, t) {
	let n = e.padding ?? 20;
	e.x = (t.minX + t.maxX) / 2, e.y = (t.minY + t.maxY) / 2, e.width = Math.max(0, t.maxX - t.minX) + n, e.height = Math.max(0, t.maxY - t.minY) + n;
}
e(Qe, "applyGroupBounds");
function $e(e) {
	let t = Je(e), n = e.filter((e) => e.isGroup && e.parentId).sort((e, n) => Xe(n, t) - Xe(e, t));
	for (let t of n) {
		let n = Ze(e.filter((e) => e.parentId === t.id));
		n && Qe(t, n);
	}
}
e($e, "recomputeNestedGroupBounds");
function et(t, n) {
	...
	let c = /* @__PURE__ */ e((e) => o + s - e, "mirror");
	for (let e of r) {
		let t = e[n];
...
e(et, "mirrorAxis");
function tt(e) {
	return !(e.nodes ?? []).some((e) => !e.isGroup) || et(e, "y");
}
e(tt, "applyBtDirectionTransform");
function nt(e, t = "LR") {
	...
	if (f.length === 0) return t === "RL" && et(e, "x"), !0;
	let p = Je(n), m = /* @__PURE__ */ new Map();
	for (let e of n) {
		if (e.isGroup) continue;
		let t = Ye(e, p);
		if (!t) continue;
	...
	return t === "RL" && et(e, "x"), !0;
}
e(nt, "applyLrDirectionTransform");
var rt = 1e-6, it = 8, at = [
	0,
	it,
	-it,
...
function ot(e, t) {
	let { nodeInfoById: n, realNodeRects: r } = de(t);
	for (let t of e) {
		...
		let a = ie(H(i, rt), rt);
		if (!a) continue;
		let { p3: o } = a, s = a.kind === "HVH", c = pe(t, n, rt);
		if (!c) continue;
		...
		for (let n of at) {
			...
			if (s) {
				...
				if (t <= g.left + rt || t >= g.right - rt) continue;
				i = {
					x: t,
					y: e
				}, a = {
				...
				if (t <= g.top + rt || t >= g.bottom - rt) continue;
				i = {
					x: e,
					y: t
				}, a = {
...
e(ot, "portSwapToLShape");
function st(t, n) {
	let r = .001, { realNodeRects: i, labelNodeRects: a } = q(n.values());
	for (let o of t) {
		...
		let v = L(d, u, r), y = R(d, u, r), b = L(f, d, r), x = R(f, d, r);
		if (!(v && x || y && b)) continue;
		...
		if (J(O, k, i, S ? [S] : [], -2) || J(O, k, a, [], -2)) continue;
		if (C) {
			let e = n.get(C), t = e ? K(e) : void 0;
			if (t && U(O, t, 2)) continue;
		}
		let A = /* @__PURE__ */ e((e, t) => `${e.x.toFixed(3)},${e.y.toFixed(3)}|${t.x.toFixed(3)},${t.y.toFixed(3)}`, "ownSegmentKey"), j = /* @__PURE__ */ new Set();
		for (let e = 0; e < c.length - 1; e++) j.add(A(c[e], c[e + 1]));
		let ee = /* @__PURE__ */ e((e, n) => {
			for (let i of t) {
				if (i === o || i.isLayoutOnly) continue;
				let t = i.points;
				if (!(!t || t.length < 2)) for (let i = 0; i < t.length - 1; i++) {
					let a = t[i], o = t[i + 1];
					if (!j.has(A(a, o)) && _e(e, n, a, o, r)) return !0;
				}
			}
			return !1;
		...
		if (l - 3 >= 0) {
			...
			if (J(e, O, i, t, -2) || ee(e, O)) continue;
		}
		let te = [
			...c.slice(0, l - 2),
			O,
			...
			if (e) {
				...
				if (t > 0 && n > 0) {
					...
					for (let e = 0; e < te.length - 1; e++) {
						let s = te[e], c = te[e + 1], l = Math.hypot(c.x - s.x, c.y - s.y), u = I(s, c, r), d = F(s, c, r);
...
e(st, "collapseShortTerminalStub");
var X = .001, Z = 8, Q = re, ct = /* @__PURE__ */ e((e, t) => F(e, t, X) || I(e, t, X), "orthogonallyAligned");
function lt(t, n) {
	let r = /* @__PURE__ */ e((e, t) => {
		...
		let a = t ? e.start : e.end, o = a ? n.get(a) : void 0, s = o ? K(o) : void 0;
		if (!o || !a || !s) return;
		let c = t ? i[0] : i[i.length - 1], l = t ? i[1] : i[i.length - 2], u = r(o, c), d = c;
		if (ct(l, u) && (d = l), F(u, d, X)) return {
			edge: e,
			edgeId: String(e.id ?? ""),
			nodeId: a,
			atStart: t,
		...
		if (!(/* @__PURE__ */ e(() => Math.abs(t.boundary.y - t.rect.top) < 1 || Math.abs(t.boundary.y - t.rect.bottom) < 1 ? I(i, t.boundary, X) && i.x >= t.rect.left + 1 && i.x <= t.rect.right - 1 : Math.abs(t.boundary.x - t.rect.left) < 1 || Math.abs(t.boundary.x - t.rect.right) < 1 ? F(i, t.boundary, X) && i.y >= t.rect.top + 1 && i.y <= t.rect.bottom - 1 : !1, "boundaryStaysOnSameFace"))()) return;
		if (t.atStart) {
			let e = r.length > 1 && P(r[1], t.railEnd, X), n = r.slice(e ? 2 : 1), o = n[0];
			return o && !ct(o, a) ? void 0 : [
				i,
				a,
				...n
			];
		}
		let o = r.length > 1 && P(r[r.length - 2], t.railEnd, X), s = r.slice(0, o ? -2 : -1), c = s[s.length - 1];
		if (!c || ct(c, a)) return [
			...s,
			a,
			i
		...
		let t = e.edge, r = H(t.points ?? []);
		if (r.length !== 2) return !1;
		let i = t.start, a = t.end, o = i ? n.get(i) : void 0, s = a ? n.get(a) : void 0;
		if (!o || !s) return !1;
		let c = o.x ?? 0, l = o.y ?? 0, u = s.x ?? 0, d = s.y ?? 0, [f, p] = r;
		return I(f, p, X) && Math.abs(l - d) < 1 && Math.abs(c - u) > 1 || F(f, p, X) && Math.abs(c - u) < 1 && Math.abs(l - d) > 1;
...
e(lt, "separateSharedRenderedTerminalLanes");
function ut(t, n) {
	let { realNodeRects: r, labelNodeRects: i } = q(n.values()), a = /* @__PURE__ */ e((e, n) => {
		...
		for (let n of t) {
			...
			let t = n.points;
			if (!(!t || t.length < 2)) {
				for (let e of s) for (let n of Q(H(t))) if (B(e, n, .5) >= Z || _e(e.a, e.b, n.a, n.b, X)) return !1;
			}
		}
		return !0;
	}, "candidateIsSafe"), o = /* @__PURE__ */ e((e, t) => {
		if (t + 4 >= e.length) return;
		let n = e[t], r = e[t + 1], i = e[t + 2], a = e[t + 3], o = e[t + 4], s = L(n, r) && R(r, i) && L(i, a) && R(a, o) && F(n, a, X) && F(n, o, X) && F(r, i, X) && (r.x - n.x) * (a.x - i.x) < 0, c = R(n, r) && L(r, i) && R(i, a) && L(a, o) && I(n, a, X) && I(n, o, X) && I(r, i, X) && (r.y - n.y) * (a.y - i.y) < 0;
		if (s || c) return H([
			...e.slice(0, t + 1),
			o,
			...e.slice(t + 5)
		...
		let l = e[t + 5], u = R(n, r) && L(r, i) && R(i, a) && L(a, o) && R(o, l) && F(n, o, X) && F(n, l, X) && F(i, a, X) && (i.x - r.x) * (o.x - a.x) < 0, d = L(n, r) && R(r, i) && L(i, a) && R(a, o) && L(o, l) && I(n, o, X) && I(n, l, X) && I(i, a, X) && (i.y - r.y) * (o.y - a.y) < 0;
		if (u || d) return H([
			...e.slice(0, t + 1),
			l,
			...e.slice(t + 6)
...
e(ut, "collapseRedundantRectangularDoglegs");
function dt(t, n) {
	let { realNodeRects: r, labelNodeRects: i } = q(n.values()), a = t.filter((e) => !e.isLayoutOnly), o = /* @__PURE__ */ e((e, t, n) => H(e === t ? n ?? [] : e.points ?? []), "pointsFor"), s = /* @__PURE__ */ e((e, t) => {
		let n = 0;
		for (let r = 0; r < a.length; r++) {
			let i = Q(o(a[r], e, t));
			for (let s = r + 1; s < a.length; s++) {
				let r = Q(o(a[s], e, t));
				for (let e of i) for (let t of r) _e(e.a, e.b, t.a, t.b, X) && n++;
			}
		}
		return n;
		...
		return r.filter((e) => {
			...
			let r = e.rect;
			return t.horizontal ? z(t.a.x, t.b.x, r.left, r.right) >= Z && t.a.y >= r.top - 2 && t.a.y <= r.bottom + 2 : z(t.a.y, t.b.y, r.top, r.bottom) >= Z && t.a.x >= r.left - 2 && t.a.x <= r.right + 2;
		});
		...
		for (let t of a) if (t !== e) {
			for (let e of l) for (let n of Q(o(t))) if (B(e, n, .5) >= Z) return !1;
		}
		return s(e, t) <= n;
	}, "candidateIsSafe");
	for (let e = 0; e < 8; e++) {
...
e(dt, "liftObstacleHuggingSameSideRails");
function ft(t, n) {
	let r = /* @__PURE__ */ e((e) => {
		...
		let n = e.a.y;
		return n <= t.top + X || n >= t.bottom - X ? !1 : z(e.a.x, e.b.x, t.left, t.right) >= Z;
...
e(ft, "liftTopLaneTitleBandsAboveRails");
function pt(t, n) {
	let r = /* @__PURE__ */ e((e) => {
		let t = e.groupTitleRect;
		if (!(!t || typeof t.left != "number" || typeof t.right != "number" || typeof t.top != "number" || typeof t.bottom != "number" || !Number.isFinite(t.left) || !Number.isFinite(t.right) || !Number.isFinite(t.top) || !Number.isFinite(t.bottom) || t.right <= t.left || t.bottom <= t.top)) return {
		...
		let n = e.a.x;
		return n <= t.left + X || n >= t.right - X ? !1 : z(e.a.y, e.b.y, t.top, t.bottom) >= Z;
...
e(pt, "shiftLeftLaneTitleBandsLeftOfRails");
function mt(t, n) {
	let { realNodeRects: r } = q(n.values()), i = t.filter((e) => !e.isLayoutOnly), a = /* @__PURE__ */ e((e, t = /* @__PURE__ */ new Map()) => H(t.get(e) ?? e.points ?? []), "replacementPointsFor"), o = /* @__PURE__ */ e((e = /* @__PURE__ */ new Map()) => {
		let t = 0;
		for (let n = 0; n < i.length; n++) {
			let r = Q(a(i[n], e));
			for (let o = n + 1; o < i.length; o++) {
				let n = Q(a(i[o], e));
				for (let e of r) for (let r of n) _e(e.a, e.b, r.a, r.b, X) && t++;
			}
		}
		return t;
	}, "crossingCount"), s = /* @__PURE__ */ e((e = /* @__PURE__ */ new Map()) => i.reduce((t, n) => t + V(a(n, e)), 0), "totalBends"), c = /* @__PURE__ */ e((e) => {
		let t = a(e);
		if (t.length < 4) return;
		let n = t[t.length - 2], r = t[t.length - 1];
		if (L(n, r, X) || R(n, r, X)) return {
			tailStart: n,
			terminal: r
		};
		...
		for (let r of i) if (r !== e) {
			for (let e of Q(t)) for (let t of Q(a(r, n))) if (B(e, t, .5) >= Z) return !0;
		}
		return !1;
...
function ht(t, n) {
	let { realNodeRects: r, labelNodeRects: i } = q(n.values()), a = t.filter((e) => !e.isLayoutOnly), o = /* @__PURE__ */ e((e, t = /* @__PURE__ */ new Map()) => H(t.get(e) ?? e.points ?? []), "replacementPointsFor"), s = /* @__PURE__ */ e((e = /* @__PURE__ */ new Map()) => {
		let t = 0;
		...
		return t;
	}, "strictCrossingCount"), c = /* @__PURE__ */ e((e = /* @__PURE__ */ new Map()) => a.reduce((t, n) => t + V(o(n, e)), 0), "totalBends"), l = /* @__PURE__ */ e((e) => {
		let t = e.start, r = e.end, i = t ? n.get(t) : void 0, a = r ? n.get(r) : void 0, o = i ? K(i) : void 0, s = a ? K(a) : void 0;
		return o && s ? {
			src: o,
			dst: s
		} : void 0;
		...
		return e;
	}, "collectExternalRails"), f = /* @__PURE__ */ e((e, t) => e.edge !== t.edge && e.axis === t.axis && e.side === t.side && z(e.min, e.max, t.min, t.max) >= Z, "railsInteract"), p = /* @__PURE__ */ e((e) => {
		...
		for (let t = 0; t < a.length; t++) {
			...
			for (let n = t + 1; n < a.length; n++) {
				...
				let s = Q(o(t, e));
				for (let e of i) for (let t of s) if (B(e, t, .5) >= Z) return !1;
			}
		}
		return !0;
	}, "candidateIsSafe");
...
e(ht, "reassignCrossingExternalRailChannels");
function gt(t, n) {
	let { realNodeRects: r, labelNodeRects: i } = q(n.values()), a = t.filter((e) => !e.isLayoutOnly), o = /* @__PURE__ */ e((e, t, n) => H(e === t ? n ?? [] : e.points ?? []), "pointsFor"), s = /* @__PURE__ */ e((e) => Q(e).reduce((e, t) => {
		...
		if (e.horizontal) {
			let n = e.a.y;
			return (Math.abs(n - t.top) < 1 || Math.abs(n - t.bottom) < 1) && z(e.a.x, e.b.x, t.left, t.right) >= Z;
		}
		if (e.vertical) {
			let n = e.a.x;
			return (Math.abs(n - t.left) < 1 || Math.abs(n - t.right) < 1) && z(e.a.y, e.b.y, t.top, t.bottom) >= Z;
		}
		return !1;
	}, "segmentRunsAlongRectBorder"), u = /* @__PURE__ */ e((e) => {
		let t = [e.start, e.end].filter((e) => !!e), r = [];
		for (let e of t) {
			let t = n.get(e), i = t ? K(t) : void 0;
			i && r.push(i);
		}
		return r;
	}, "endpointRectsFor"), d = /* @__PURE__ */ e((e, t) => {
		if (t + 3 >= e.length) return [];
		let n = e[t], r = e[t + 1], i = e[t + 2], a = e[t + 3], o = L(n, r, X) && R(r, i, X) && L(i, a, X), s = R(n, r, X) && L(r, i, X) && R(i, a, X);
		if (!o && !s || !(o ? Math.sign(r.x - n.x) !== Math.sign(a.x - i.x) : Math.sign(r.y - n.y) !== Math.sign(a.y - i.y))) return [];
		let c = F(n, a, X) || I(n, a, X) ? [] : [{
			x: n.x,
			y: a.y
		}, {
			x: a.x,
		...
		return l.map((e) => Se(H(e))).filter((e) => {
			if (Q(e).length !== e.length - 1 || !e.some((e) => P(e, a, X))) return !1;
		...
		for (let n of a) if (n !== e) {
			for (let e of Q(t)) for (let t of Q(o(n))) if (B(e, t, .5) >= Z) return !1;
		}
		return c(e, t) <= n;
	}, "candidateIsSafe");
	for (let e = 0; e < 8; e++) {
...
e(gt, "shortcutRedundantOrthogonalJogs");
function _t(t, n) {
	let r = [];
	for (let e of n.values()) {
		if (e.isGroup || e.isEdgeLabel) continue;
		let t = e.x ?? 0, n = e.y ?? 0, i = K(e);
		i && r.push({
			id: String(e.id ?? ""),
			cx: t,
			cy: n,
	...
	let i = new Map(r.map((e) => [e.id, e])), a = r.map((e) => ({
	...
	}, c = t.filter((e) => !e.isLayoutOnly), l = new Map(c.map((e, t) => [e, t])), u = /* @__PURE__ */ e((e) => {
		let t = e === "left" || e === "top" ? -1 : 1, n = [];
		for (let r = 0; r <= 2; r++) n.push(s[e] + t * 20 * r);
		return n;
	}, "outwardTracksForSide"), d = /* @__PURE__ */ e((e, t = /* @__PURE__ */ new Map()) => H(t.get(e) ?? e.points ?? []), "replacementPointsFor"), f = /* @__PURE__ */ e((e, t) => {
		let n = 0;
		for (let r of e) for (let e of t) _e(r.a, r.b, e.a, e.b, X) && n++;
		return n;
	}, "crossingCountBetweenSegments"), p = /* @__PURE__ */ e((e, t) => f(Q(e), Q(t)), "crossingCountBetweenPaths"), m = /* @__PURE__ */ e((t = /* @__PURE__ */ new Map()) => {
	...
	}, "crossingSnapshot"), h = /* @__PURE__ */ e((e, t) => {
		...
		return e.count - r + i;
	}, "crossingCountWithReplacements"), g = /* @__PURE__ */ e((e) => {
		let t = /* @__PURE__ */ new Map();
		...
		return n;
	}, "crossingComponents"), _ = /* @__PURE__ */ e((e) => [e.start, e.end].filter((e) => !!e), "endpointIdsFor"), v = /* @__PURE__ */ e((e) => {
		let t = [];
		for (let n of g(e)) {
		...
		return t;
	}, "pairSearchGroups"), y = /* @__PURE__ */ e((e, t, n) => h(e, /* @__PURE__ */ new Map([[t, n]])), "crossingCountWithSingleReplacement"), b = /* @__PURE__ */ e((e) => {
		let t = /* @__PURE__ */ new Map();
		for (let n of e.pairs) t.set(n.first, (t.get(n.first) ?? 0) + n.count), t.set(n.second, (t.get(n.second) ?? 0) + n.count);
		return t;
	}, "currentCrossingsByEdge"), x = /* @__PURE__ */ e((e) => e.slice(1).reduce((t, n, r) => {
		let i = e[r];
		return t + Math.abs(n.x - i.x) + Math.abs(n.y - i.y);
	}, 0), "pathLength"), S = /* @__PURE__ */ e((e = /* @__PURE__ */ new Map()) => c.reduce((t, n) => t + V(d(n, e)), 0), "totalBends"), C = /* @__PURE__ */ e((e = /* @__PURE__ */ new Map()) => c.reduce((t, n) => t + x(d(n, e)), 0), "totalLength"), w = /* @__PURE__ */ e((e, t, n = /* @__PURE__ */ new Map()) => {
		let r = Q(t);
		for (let t of c) if (t !== e) {
			for (let e of r) for (let r of Q(d(t, n))) if (B(e, r, .5) >= Z) return !0;
		}
		return !1;
	}, "pathHasSegmentConflict"), T = /* @__PURE__ */ e((e, t) => {
		let n = [e.start, e.end].filter((e) => !!e);
		for (let e of Q(t)) if (J(e.a, e.b, a, n, -2)) return !0;
		return !1;
	}, "pathHitsNode"), E = /* @__PURE__ */ e((e, t) => {
		let n = Se(H(t));
		Q(n).length === n.length - 1 && e.push(n);
	}, "pushOrthogonalCandidate"), D = /* @__PURE__ */ e((e) => e === "left" || e === "right", "sideIsHorizontal"), O = /* @__PURE__ */ e((e, t, n) => {
		switch (t) {
	...
	}, "localTrackForSameSide"), k = /* @__PURE__ */ e((e, t, n, r) => {
		let i = n === "left" || n === "top" ? -1 : 1, a = [O(t, n, r), s[n]];
		for (let o of a) for (let a = 0; a <= 2; a++) E(e, ue(t, n, r, o + i * 20 * a));
	}, "addSameSideCandidates"), A = /* @__PURE__ */ e((e, t, n, r, i) => {
	...
	}, "addHorizontalToVerticalCandidates"), j = /* @__PURE__ */ e((e, t, n, r, i) => {
		for (let a of u(n)) for (let n of u(i)) E(e, [
			t,
			{
				x: t.x,
	...
	}, "addVerticalToHorizontalCandidates"), ee = /* @__PURE__ */ e((e, t, n, r, i) => {
	...
	}, "addHorizontalPairCandidates"), te = /* @__PURE__ */ e((e, t, n, r, i) => {
	...
	}, "addVerticalDepartureOuterTrackCandidates"), P = /* @__PURE__ */ e((e, t, n, r) => {
		let i = [...u("left"), ...u("right")], a = [...u("top"), ...u("bottom")];
		for (let s of o) {
			let o = ce(r, s), c = s === "left" || s === "right" ? u(s) : i;
			for (let r of a) {
				E(e, [
					t,
					n,
	...
	}, "addHorizontalDepartureOuterTrackCandidates"), F = /* @__PURE__ */ e((e) => {
		let t = e.start, n = e.end, r = n ? i.get(n) : void 0;
		if (!t || !r) return [];
		let a = H(e.points ?? []);
		if (a.length < 4) return [];
		let o = a[0], s = a[1], c = [];
		return R(o, s, X) ? N(c, o, s, r) : L(o, s, X) && P(c, o, s, r), c;
	}, "terminalPreservingOuterTrackCandidates"), I = /* @__PURE__ */ e((e) => {
		let t = e.start, n = e.end, r = t ? i.get(t) : void 0, a = n ? i.get(n) : void 0;
		if (!r || !a) return [];
		let s = [];
		for (let e of o) {
			let t = ce(r, e);
			for (let n of o) s.push(...ne(t, e, ce(a, n), n));
		}
		return s.push(...F(e)), s;
	}, "candidatePathsFor"), z = /* @__PURE__ */ e(() => new Map(c.map((e) => [e, Q(d(e))])), "currentSegmentsByEdge"), re = /* @__PURE__ */ e((e, t, n) => {
		let r = /* @__PURE__ */ new Set();
		for (let i of c) {
			if (i === e) continue;
			let a = n.get(i) ?? Q(d(i));
			t.some((e) => a.some((t) => B(e, t, .5) >= Z)) && r.add(i);
		}
		return r;
	}, "sharedTrackConflictsFor"), ie = /* @__PURE__ */ e((e, t, n, r) => {
		let i = /* @__PURE__ */ new Set();
		return I(e).map((e) => Se(H(e))).filter((t) => {
			if (T(e, t)) return !1;
			let n = t.map((e) => `${e.x.toFixed(3)},${e.y.toFixed(3)}`).join("|");
			return i.has(n) || t.length < 2 ? !1 : (i.add(n), !0);
		}).map((i) => {
			let a = Q(i), o = 0;
			for (let t of c) t !== e && (o += f(a, n.get(t) ?? Q(d(t))));
			return {
				candidate: i,
				candidateSegments: a,
			...
			sharedTrackConflicts: re(e, t.candidateSegments, n),
			totalBends: t.totalBends,
			length: t.length
		}));
	}, "pairCandidatesFor"), ae = /* @__PURE__ */ e((e, t, n, r, i, a) => {
		let o = 0;
		...
		return e.count - o + s;
	}, "pairCrossingCount"), U = /* @__PURE__ */ e((e, t) => {
		for (let n of e.sharedTrackConflicts) if (n !== t) return !1;
		return !0;
	}, "conflictsOnlyWith"), W = /* @__PURE__ */ e((e, t) => e.segments.some((e) => t.segments.some((t) => B(e, t, .5) >= Z)), "candidatesShareTrack"), G = /* @__PURE__ */ e((e, t, n, r) => U(t, n.edge) && U(r, e.edge) && !W(t, r), "pairCandidatesAreCompatible"), oe = /* @__PURE__ */ e((e, t, n, r, i) => {
		let a = ae(e.current, t.edge, n, r.edge, i, e.baseSegments);
	...
	}, "scorePairReplacement"), se = /* @__PURE__ */ e((e, t) => e.crossings < t.crossings || e.crossings === t.crossings && (e.bends < t.bends || e.bends === t.bends && e.length < t.length), "pairScoreIsBetter"), de = /* @__PURE__ */ e((e, t, n, r) => {
		let i = r;
		for (let r of t.candidates) for (let a of n.candidates) {
			if (!G(t, r, n, a)) continue;
			let o = oe(e, t, r, n, a);
			o && se(o, i) && (i = o);
		}
		return i;
	}, "bestScoreForOptionPair"), q = /* @__PURE__ */ e((e) => {
		let t = S(), n = C(), r = z(), i = b(e), a = new Map(c.map((e) => [e, V(d(e))])), o = new Map(c.map((e) => [e, x(d(e))])), s = /* @__PURE__ */ new Map(), l = v(e);
		...
		for (let t of l) {
			...
			for (let e = 0; e < r.length; e++) {
				...
				for (let i = e + 1; i < r.length; i++) {
					let e = r[i];
					(n.has(t.edge) || n.has(e.edge)) && (u = de(f, t, e, u));
				}
			}
		}
	...
	for (let e = 0; e < 4; e++) {
		...
		for (let o of e.edges) {
			let s = V(d(o), X);
			for (let c of I(o)) {
				let l = T(o, c), u = !l && w(o, c), d = y(e, o, c), f = V(c, X);
...
e(_t, "resolveRenderedOrthogonalCrossings");
var vt = .001, yt = 8;
function bt(t, n) {
	let { nodeInfoById: r, realNodeRects: i } = de(n), a = [
		"top",
		"bottom",
		"left",
		"right"
		...
		let i = [], a = le(e, t, n, r, 20, vt);
		return a && i.push(a), t === r && i.push(ue(e, t, n, o[t])), i;
		...
		let i = 0, a = re(e, vt), o = n.start, s = n.end;
		for (let e of t) {
			if (e === n || e.isLayoutOnly) continue;
			let t = e.start, c = e.end;
			if (!r && o && s && (t === o || t === s || c === o || c === s)) continue;
			let l = e.points;
			if (!(!l || l.length < 2)) for (let e of a) for (let t of re(l, vt)) {
				if (me(e.a, e.b, t.a, t.b, vt, vt)) {
					i++;
					continue;
				}
				B(e, t, vt) >= yt && i++;
	...
	let p = /* @__PURE__ */ e((e, t, n) => d.get(e)?.some((e) => e.edgeId !== n && e.side === t) ?? !1, "faceIsClaimed");
	for (let e of t) {
		if (e.isLayoutOnly) continue;
		let t = e.points;
...
e(bt, "simplifyDetouredEdges");
var xt = .001, St = 10, Ct = 7;
function wt(e, t) {
	let n = t ? 0 : e.length - 1, r = t ? 1 : -1, i = e[n], a = e[n + r];
	if (!i || !a) return;
	let o = a.x - i.x, s = a.y - i.y;
	if (!(Math.abs(o) + Math.abs(s) < xt)) {
		if (Math.abs(s) <= xt) {
			let e = i.x + Math.sign(o) * St;
			return {
				left: Math.min(i.x, e),
...
function Et(e, t) {
	let n = H(t);
	return [wt(n, !0), wt(n, !1)].some((t) => t && G(e, Tt(t)));
}
e(Et, "labelOverlapsOwnMarker");
function Dt(t, n) {
	let r = [];
	for (let e of t) {
		if (e.isLayoutOnly) continue;
	...
	let o = /* @__PURE__ */ e((e, t) => {
		let n = oe(t, 3);
		for (let { nodeId: t, rect: r } of i) if (t !== e && G(n, r)) return !0;
		return !1;
	}, "labelOverlapsForeignNode"), s = /* @__PURE__ */ e((e, t) => {
		let n = oe(t, 3);
		for (let t of r) if (t.edgeId !== e && ae(t.p1, t.p2, n)) return !0;
		return !1;
	}, "labelOverlapsForeignEdge"), c = /* @__PURE__ */ e((e, t, n) => o(e, n) || s(t, n), "labelOverlapsAnything"), l = [], u = /* @__PURE__ */ e((e) => {
		for (let { id: t, rect: n } of a) if (W(n, e)) return t;
	}, "findContainingLane"), d = /* @__PURE__ */ e((e, t) => l.some((n) => n.labelId !== e && G(t, n.rect)), "overlapsPlacedLabel");
	for (let r of t) {
		...
		for (let e = 0; e < f.length - 1; e++) {
			...
			r < xt && i < xt || r >= xt && i >= xt || h.push({
				idx: e,
				length: r + i,
				orientation: r >= xt ? "horizontal" : "vertical",
				midX: (t.x + n.x) / 2,
				midY: (t.y + n.y) / 2
			});
		}
		if (h.length === 0) continue;
		let g = h.length >= 3 ? h.filter((e) => e.idx > 0 && e.idx < h.length - 1) : h, _ = g.length > 0 ? g : h, v = p >= m ? "horizontal" : "vertical", y = /* @__PURE__ */ e((e) => [...e].sort((e, t) => {
		...
		}, "anchorAtT"), w = /* @__PURE__ */ e((e, t, n) => Math.min(n, Math.max(t, e)), "clamp"), T = /* @__PURE__ */ e((e, t) => e.midX >= t.left - xt && e.midX <= t.right + xt && e.midY >= t.top - xt && e.midY <= t.bottom + xt, "pointInsideRectInclusive"), E = /* @__PURE__ */ e((e) => {
			let t = se(e.midX, e.midY, p, m), n = u(t);
			if (n) return {
				laneId: n,
				anchor: e,
				rect: t
			...
			let l = {
				midX: w(e.midX, i, o),
				midY: w(e.midY, s, c)
			}, d = se(l.midX, l.midY, p, m);
			return T(e, d) ? {
				laneId: r.id,
				anchor: l,
				rect: d
			} : void 0;
		}, "placementForAnchor"), D = /* @__PURE__ */ e((e, t, n) => e.orientation === "horizontal" ? Math.abs(t.midX - n.x) : Math.abs(t.midY - n.y), "distanceAlongSegment"), O = /* @__PURE__ */ e((e, t) => {
			let n = (e.orientation === "horizontal" ? p / 2 : m / 2) + 12;
			if (e === b) {
				let r = f[e.idx];
				if (D(e, t, r) + xt < n) return !1;
			}
			if (e === x) {
				let r = f[e.idx + 1];
				if (D(e, t, r) + xt < n) return !1;
			...
			return !0;
		}, "labelClearsTerminalEndpoints"), k = /* @__PURE__ */ e((e) => {
			let n = y(e);
			for (let e of n) for (let n of S) {
				let i = C(e, n);
				if (!O(e, i)) continue;
				let a = E(i);
				if (a && !Et(a.rect, f) && !d(t, a.rect) && !c(t, r.id, a.rect)) return {
					laneId: a.laneId,
					anchor: a.anchor
				};
			}
		}, "tryPool"), A = /* @__PURE__ */ e((e, n, i = !1) => {
			let a = y(e);
			for (let e of a) {
				let a = {
					midX: e.midX,
				...
				if (n && !O(e, a)) continue;
				let c = E(a);
				if (c && !Et(c.rect, f) && !d(t, c.rect) && !o(t, c.rect) && (i || !s(r.id, c.rect))) return {
					laneId: c.laneId,
					anchor: c.anchor
				};
			}
		...
		if (j) {
			...
			let e = se(j.anchor.midX, j.anchor.midY, p, m), n = l.findIndex((e) => e.labelId === t);
			n >= 0 ? l[n] = {
				labelId: t,
				rect: e
			} : l.push({
...
e(Dt, "anchorLabelsToPolyline");
var Ot = 1e-6, kt = 4, At = 3;
function jt(e, t) {
	return e < t ? `${e}::${t}` : `${t}::${e}`;
}
e(jt, "pairKey");
function Mt(t, n) {
	let { nodeInfoById: r, realNodeRects: i } = de(n), a = /* @__PURE__ */ new Map();
	for (let e of n) {
		let t = e.id;
	...
	let o = /* @__PURE__ */ e((n, r, i, o) => {
		let s = jt(r, i), c = 0, l = /* @__PURE__ */ e((e) => {
			if (!e) return;
			let t = a.get(e);
			if (!t) return;
	...
	for (let e of t) {
		...
		if (J(p, m, i, [s, c], 1)) continue;
		let h = o(e, s, c, d ? "x" : "y"), g = h > kt ? h : kt, _ = [
			0,
			g,
			-g
		];
		for (let n of _) {
			...
			if (!J(r, a, i, [s, c], 1) && !ge(r, a, t, e, { epsilon: Ot })) {
				e.points = [r, a];
				break;
			}
		}
...
e(Mt, "straightenCollinearSiblingDetours");
function Nt(t, n) {
	let r = .001, { realNodeRects: i, labelNodeRects: a } = q(n.values()), o = /* @__PURE__ */ e((e, t) => re(t, r).map((n) => ({
		...n,
		edge: e,
		...
		for (let n of t) {
			if (n.isLayoutOnly) continue;
			let t = n.points;
			!t || t.length < 2 || e.push(...o(n, H(t)));
		}
		return e;
	}, "allSegments"), c = /* @__PURE__ */ e((e, t) => e.horizontal && t.horizontal ? z(e.a.x, e.b.x, t.a.x, t.b.x) >= 8 && Math.abs(e.a.y - t.a.y) < 7 : e.vertical && t.vertical ? z(e.a.y, e.b.y, t.a.y, t.b.y) >= 8 && Math.abs(e.a.x - t.a.x) < 7 : !1, "hasCrowdedParallelTrack"), l = /* @__PURE__ */ e((e, n) => {
		...
		for (let n of t) {
			...
			let t = n.points;
			if (!(!t || t.length < 2)) {
				for (let e of u) for (let i of o(n, H(t))) if (c(e, i) || _e(e.a, e.b, i.a, i.b, r)) return !1;
			}
		}
		return !0;
	}, "candidateIsSafe"), u = /* @__PURE__ */ e((e, t) => {
		let n = H(e.edge.points ?? []);
		...
		let i = t.start ? n.get(t.start) : void 0, a = t.end ? n.get(t.end) : void 0, o = i ? K(i) : void 0, s = a ? K(a) : void 0, c = r.slice(e.index + 2);
...
e(Nt, "nudgeSharedInteriorSubpaths");
function Pt(e, t, n, r) {
	...
	return d > p && d < .99 && f > p && f < .99;
}
e(Pt, "segmentsIntersect");
function Ft(e) {
	...
	for (let e of r) {
		...
		for (let e of a) if (e.nodeId !== n && e.nodeId !== r && !(s && e.nodeId === s)) {
			for (let n = 0; n < t.length - 1; n++) if (ae(t[n], t[n + 1], e, -1)) {
				i.push({
	...
	for (let e = 0; e < o.length; e++) for (let t = e + 1; t < o.length; t++) {
		let n = o[e], r = o[t];
		if (n.edgeId !== r.edgeId && n.start !== r.start && n.start !== r.end && n.end !== r.start && n.end !== r.end && Pt(n.p1, n.p2, r.p1, r.p2)) {
	...
	if (i.length > 0) {
		...
		t.warn(`[SWIMLANE_VALIDATE] ${i.length} issue(s) detected: ${e} edge-node overlap(s), ${n} edge crossing(s)`);
		for (let e of i) t.warn(`[SWIMLANE_VALIDATE]   ${e.type}: ${e.detail}`);
	}
	return i;
}
e(Ft, "validateSwimlanesLayout");
function It(t, n) {
	let r = t.nodes ?? [], i = t.edges ?? [], a = r.filter((e) => !e.isGroup);
	if ((n === "LR" || n === "RL") && a.length > 0 && !nt(t, n) || n === "BT" && a.length > 0 && !tt(t)) return;
	for (let e of i) {
		if (e.isLayoutOnly) continue;
		let t = e.points;
		!t || t.length < 2 || (e.points = Se(xe(t)));
	}
	bt(i, r), Mt(i, r), ot(i, r);
	let o = /* @__PURE__ */ new Map();
	for (let e of r) o.set(String(e.id), e);
	Dt(i, o), Oe(i, o), st(i, o), Nt(i, o), lt(i, o), ut(i, o), dt(i, o), mt(i, o);
	let s = /* @__PURE__ */ e(() => {
		_t(i, o), ht(i, o), gt(i, o), Dt(i, o), qe(i, o), dt(i, o), Dt(i, o), qe(i, o);
	}, "finalizeRenderedEdges");
	s(), Nt(i, o), s(), ft(i, o), pt(i, o), ft(i, o), pt(i, o);
}
e(It, "postProcessSwimlaneLayout");
function Lt(e) {
...
e(zt, "buildSuccessorMap");
function Bt(e) {
	let t = zt(e);
	for (let e of t.values()) e.sort((e, t) => e.localeCompare(t));
	return t;
}
e(Bt, "buildSortedSuccessorMap");
function Vt(e) {
	let t = /* @__PURE__ */ new Map();
...
e(Ut, "buildPredecessorSuccessorMaps");
function Wt(e, t, n, r) {
	let i = 0;
...
e(Wt, "buildLayersFromRanks");
function Gt(e) {
	let t = Vt(e), n = Ht(t), r = [], i = Bt(e);
	for (; n.length;) {
		let e = n.shift();
		r.push(e);
...
e(Gt, "topoSortIfAcyclic");
function Kt(e) {
	let t = /* @__PURE__ */ new Map(), n = 0;
	for (let r of e) t.set(r, n), n++;
	return t;
}
e(Kt, "buildLayerIndex");
function qt(t) {
...
e(qt, "countInversions");
function Jt(t) {
	let n = Lt(t), r = /* @__PURE__ */ new Map();
...
e(Qt, "resolveTopLaneOrder");
var $t = { EPSILON: 1e-6 }, en = {
...
function nn(t, n) {
	let r = Lt(t), i = n?.laneOf ?? (() => null), a = n?.rankHint, { preds: o } = Ut(r);
	for (let e of o.values()) e.sort((e, t) => e.localeCompare(t));
	let s = Gt(r) ?? [...r.nodes].sort((e, t) => e.localeCompare(t)), c = /* @__PURE__ */ new Map();
...
e(on, "assignComponents");
function sn(t) {
	let n = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map(), i = [], a = [], o = 0, s = /* @__PURE__ */ e((e, c) => {
		n.set(e, ++o), r.set(e, o);
		for (let l of t.get(e) ?? []) l !== c && (n.has(l) ? (n.get(l) ?? 0) < (n.get(e) ?? 0) && (i.push([e, l]), r.set(e, Math.min(r.get(e) ?? o, n.get(l) ?? o))) : (i.push([e, l]), s(l, e), r.set(e, Math.min(r.get(e) ?? o, r.get(l) ?? o)), (r.get(l) ?? 0) >= (n.get(e) ?? 0) && a.push(cn(e, l, i, a.length))));
	}, "visit");
...
function ln(t, n, r) {
	...
	let h = /* @__PURE__ */ new Map(), g = /* @__PURE__ */ e((e, t) => {
		if (t.size !== 0) for (let [n, r] of t) e.set(n, (e.get(n) ?? 0) + r);
	}, "mergeInto"), _ = /* @__PURE__ */ new Set(), v = /* @__PURE__ */ e((e) => {
		let t = a.get(e);
		_.add(e);
...
e(ln, "computeSubtreeCrossCounts");
function un(t, n, r) {
...
function fn(t, n, r, i) {
	...
	let o = Array.from({ length: a + 1 }, () => []), s = /* @__PURE__ */ new Set(), c = /* @__PURE__ */ e((e) => {
		if (s.has(e)) return;
		s.add(e);
		let t = r[e] ?? 0;
		o[t] || (o[t] = []), o[t].push(e);
...
function hn(e, t, n) {
	...
	let o = ln(e, t, r), s = [...a].sort(dn(t)), c = mn(i, t, o, un(s, i, t)), l = fn(s, e.nodes, t, c);
	return l = pn(l), l;
}
e(hn, "buildMultitreeLayerOrder");
function gn(e, t, n) {
	let r = new Set(e), i = new Set(t), a = Kt(t), o = [];
	for (let e of n) r.has(e.src) && i.has(e.dst) && o.push(a.get(e.dst));
	return qt(o);
}
e(gn, "countCrossingsBetweenAdjacent");
function _n(e, t, n) {
	let r = [];
	for (let e of t) {
	...
	let i = 0;
	for (let t = 0; t + 1 < e.length; t++) i += gn(e[t], e[t + 1], r);
	return i;
}
e(_n, "totalCrossings");
function vn(e, t) {
	let n = { ...t }, { preds: r } = Ut(e), i = Xt(e), a = _n(hn(e, n, i), e.edges, n), o = en.MAX_CROSSING_OPTIMIZATION_PASSES;
	for (let t = 0; t < o; t++) {
...
e(vn, "optimizeRanksByCrossings");
function yn(e, t) {
	let n = Xt(e), r = [...e.nodes].sort((e, n) => (t[e] ?? 0) - (t[n] ?? 0) || e.localeCompare(n));
	for (let i of r) {
		let r = n(i);
		if (!r) continue;
...
e(yn, "adjustCrossLaneSources");
function bn(e, t) {
	let n = Lt(e), r = Gt(n) ?? [...n.nodes].sort(), i = t?.compactSingleInput ?? !1, a = Xt(n), o = /* @__PURE__ */ Object.create(null);
	for (let e of r) {
		let r = Rt(n, e), s = t?.ignoreCrossLaneEdges ? r.filter((t) => {
			let n = a(t.src), r = a(e);
			return !n || !r || n === r;
		}) : r;
		if (s.length === 0) o[e] = 0;
	...
	return (t?.optimizeRanksByCrossings ?? !1) && (o = vn(n, o)), t?.ignoreCrossLaneEdges && yn(n, o), {
		layers: hn(n, o, a),
		rankOf: o,
		dummy: /* @__PURE__ */ new Set()
	};
}
e(bn, "assignLayers_LongestPath");
function xn(t, n) {
	let r = Lt(t), i = { ...bn(r, {
	...
	}).rankOf }, a = Xt(r), { preds: o, succs: s } = Ut(r, (e) => {
		if (n?.ignoreCrossLaneEdges) {
			let t = a(e.src), n = a(e.dst);
			if (t && n && t !== n) return !1;
		}
		return !0;
	}), c = Gt(r) ?? [...r.nodes], l = [...c].reverse(), u = /* @__PURE__ */ e((e, t) => {
		let n = 0;
	...
	}, "clampFeasible"), d = en.GRAVITY_ITERATIONS, f = /* @__PURE__ */ e((e) => {
		let t = !1;
		for (let n of e) {
			let e = o.get(n) ?? [], r = s.get(n) ?? [];
			if (e.length === 0 && r.length === 0) continue;
			let a = e.length > 0 ? e.reduce((e, t) => e + (i[t] ?? 0) + 1, 0) / e.length : i[n] ?? 0, c = r.length > 0 ? r.reduce((e, t) => e + (i[t] ?? 0) - 1, 0) / r.length : i[n] ?? 0, l = Math.round((a + c) / 2), d = u(n, l);
			d !== i[n] && (i[n] = d, t = !0);
		}
		return t;
	}, "relaxOrder");
...
e(xn, "assignLayers_Gravity");
function Sn(e) {
	let t = Vt(e), n = Bt(e), r = Ht(t), i = [];
	for (; r.length > 0;) {
		let e = [];
		for (let a of r) {
			i.push(a);
...
e(Sn, "topoSortByGenerationIfAcyclic");
function Cn(t, n) {
	let r = Lt(t), i = n?.direction === "LR" ? Sn(r) ?? [...r.nodes].sort() : Gt(r) ?? [...r.nodes].sort(), a = Xt(r), o = /* @__PURE__ */ e((e) => a(e) ?? e, "laneOf"), s = /* @__PURE__ */ Object.create(null), c = /* @__PURE__ */ new Map(), l = /* @__PURE__ */ e((e, t) => n?.ignoreCrossLaneEdges ?? !0 ? +(o(e) === o(t)) : 1, "edgeWeight");
	for (let e of i) {
	...
	return {
		layers: Wt(r, i, s, { skipGroups: !0 }),
		rankOf: s,
		dummy: /* @__PURE__ */ new Set()
	};
}
e(Cn, "assignLayers_LaneAwareCompact");
function wn(t, n) {
	let r = Lt(n), { rankOf: i } = t, a = t.layers.map((e) => [...e]), o = new Set(t.dummy ? [...t.dummy] : []), s = 0, c = new Map(r.nodeById), l = /* @__PURE__ */ e((e) => {
...
e(Dn, "neighborPositionsFor");
function On(e, t, n) {
	let r = n.get(e) ?? 0, i = n.get(t) ?? 0;
	return r === i ? e.localeCompare(t) : r - i;
}
e(On, "currentOrderTieBreak");
function kn(e, t, n) {
	let r = new Set(e), i = new Set(t), a = Kt(e), o = Kt(t), s = [];
	for (let e of n) r.has(e.src) && i.has(e.dst) && s.push({
		u: a.get(e.src),
		v: o.get(e.dst)
	});
	return s.sort((e, t) => e.u === t.u ? e.v - t.v : e.u - t.u), qt(s.map((e) => e.v));
}
e(kn, "countCrossingsBetweenAdjacent");
function An(e, t, n) {
	return [...e].sort((e, r) => {
		let i = Tn(t.get(e) ?? []), a = Tn(t.get(r) ?? []);
		return i === a ? On(e, r, n) : isFinite(i) ? isFinite(a) ? i - a : -1 : 1;
	});
}
e(An, "sortByHeuristic");
function jn(e, t, n, r, i, a) {
	let o = Kt(e), s = Kt(t), c = Dn(t, o, n, r);
	if (!i || !a || a.length === 0) return An(t, c, s);
	...
	if (d && d.length > 0) {
		let e = An(d, c, s);
		for (let t of e) {
			let e = En(c.get(t) ?? []), n = u.length;
			if (isFinite(e)) {
				for (let [t, r] of u.entries()) if (e < En(c.get(r) ?? [])) {
					n = t;
					break;
				}
			}
...
e(Mn, "transposeImprove");
function Nn(e, t, n) {
	let r = e.layers.map((e) => [...e]), i = t.edges, a = Xt(t), o = Qt(t, n?.laneOrder);
	for (let e = 0; e < 3; e++) {
		for (let e = 1; e < r.length; e++) r[e] = jn(r[e - 1], r[e], i, "down", a, o), r[e] = Mn(r[e - 1], r[e], i, r[e + 1], a);
		for (let e = r.length - 2; e >= 0; e--) r[e] = jn(r[e + 1], r[e], i, "up", a, o), r[e] = Mn(r[e + 1], r[e], i, r[e - 1], a);
	}
	return { layers: r };
}
e(Nn, "orderLayers");
function Pn(t, n, r) {
	let i = r?.layerGap ?? tn.DEFAULT_LAYER_GAP, a = r?.nodeGap ?? tn.DEFAULT_NODE_GAP, o = r?.laneGap ?? a * 2, s = r?.direction ?? "TB", c = s === "LR" || s === "RL", l = t.layers, u = /* @__PURE__ */ Object.create(null), d = /* @__PURE__ */ Object.create(null), f = /* @__PURE__ */ e((e) => n.nodeById.get(e), "getNode"), p = /* @__PURE__ */ e((e) => f(e)?.width ?? 0, "getWidth"), m = /* @__PURE__ */ e((e) => f(e)?.height ?? 0, "getHeight"), h = Xt(n), g = Qt(n, r?.laneOrder), _ = l.map((e) => e.reduce((e, t) => Math.max(e, m(t)), 0)), v = [];
...
e(Bn, "laneArrangementCost");
function Vn(e) {
	let t = Zt(e);
	if (t.length < 2) return [];
	let n = new Map(t.map((e, t) => [e, t])), r = Xt(e), i = /* @__PURE__ */ new Map();
...
e(Vn, "buildWeightedLaneEdges");
function Hn(e, t, n) {
	let r = [...e], i = Bn(r, t), a = !0, o = 0, s = Math.max(1, r.length);
	for (; a && o < s;) {
		a = !1, o++;
...
function Gn(e, t = {}) {
	...
	let i = new Map(n.map((e, t) => [e, t])), a = Hn(n, r, i), o = Math.max(0, t.restarts ?? Fn);
	for (let e = 0; e < o; e++) {
		let t = Hn(Rn(n, Wn(n, r, e)), r, i);
		Un(t, a) && (a = t);
	}
	return a.order;
}
e(Gn, "optimizeTopLaneOrder");
function Kn(e, t) {
	let n = t?.ignoreCrossLaneEdges ?? !0, r = t?.optimizeRanksByCrossings ?? !0, i = Lt(e), a = t?.automaticLaneOrdering ? Gn(i, { restarts: Fn }) : void 0, o = Jt(i), s = o.acyclic, { layering: c, graphWithDummies: l } = wn(n ? Cn(s, {
		compactSingleInput: t?.compactSingleInput ?? en.DEFAULT_COMPACT_SINGLE_INPUT,
	...
	}), s), u = Nn(c, l, { laneOrder: a }), d = Pn(u, l, {
		layerGap: t?.layerGap,
		nodeGap: t?.nodeGap,
		direction: t?.direction,
		laneOrder: a
...
function nr(t, n) {
	...
	}, "portForSide"), m = /* @__PURE__ */ e((e, t, n) => p(e, $n(e, t, n ? "bottom" : "top")), "getOrthogonalPort"), h = [], g = [], _ = /* @__PURE__ */ new Set(), v = 1e3, y = /* @__PURE__ */ e((e, t, n) => {
	...
	}).map((e) => e.idx), x = /* @__PURE__ */ e((e, t, n, r) => {
		let i = Math.min(e.x, t.x), a = Math.max(e.x, t.x), o = Math.min(e.y, t.y), s = Math.max(e.y, t.y);
	...
	let w = /* @__PURE__ */ e((e, t) => $n(e, t, "bottom"), "determineSide"), T = /* @__PURE__ */ new Map();
	...
	let E = /* @__PURE__ */ e((e) => e.srcSide === "top" || e.srcSide === "bottom" ? e.absDx === 0 ? Infinity : e.absDy / e.absDx : e.absDy === 0 ? Infinity : e.absDx / e.absDy, "preferenceStrength"), D = /* @__PURE__ */ e((e) => e.srcSide === "top" || e.srcSide === "bottom" ? e.dxSign >= 0 ? "right" : "left" : e.dySign >= 0 ? "bottom" : "top", "secondarySide"), O = /* @__PURE__ */ new Map();
	for (let e of T.values()) {
		let t = `${e.srcId}:${e.srcSide}`;
		O.has(t) || O.set(t, []), O.get(t).push(e);
	}
	let k = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ e((e, t) => `${e}:${t}`, "loadKey");
	...
	let M = /* @__PURE__ */ e((e) => !!a[e]?.labelNodeId, "edgeHasLabelNode"), ne = /* @__PURE__ */ e((e, t) => e ? (S.get(`${e}:${t}:src`) ?? []).some(({ edgeIdx: e }) => M(e)) || (S.get(`${e}:${t}:dst`) ?? []).some(({ edgeIdx: e }) => M(e)) : !1, "faceHasLabelNode"), N = /* @__PURE__ */ e((e, t, n) => t === "top" || t === "bottom" ? {
		x: e.x + n,
		y: e.y
	} : {
		x: e.x,
		y: e.y + n
	}, "applyPortOffset"), P = /* @__PURE__ */ e((e, t, n) => {
		let r = T.get(e), i = {
			x: n.x ?? 0,
			y: n.y ?? 0
		}, a = {
	...
	for (let t of b) {
		...
		if (!r || !i) continue;
		let { pSrcPort: s, pDstPort: u, srcSide: p, dstSide: m } = P(t, r, i), v = { ...s }, b = { ...u }, w = p === "top" || p === "bottom", T = m === "top" || m === "bottom";
		w ? v.y = s.y > (r.y ?? 0) ? s.y + Zn : s.y - Zn : v.x = s.x > (r.x ?? 0) ? s.x + Zn : s.x - Zn, T ? b.y = u.y > (i.y ?? 0) ? u.y + Zn : u.y - Zn : b.x = u.x > (i.x ?? 0) ? u.x + Zn : u.x - Zn;
		let E = /* @__PURE__ */ e((e, t) => {
			...
			if (i) {
				let i = e.y > (t.y ?? 0);
				return {
					x: (n.x ?? 0) >= e.x ? r.maxX + Jn : r.minX - Jn,
					y: i ? r.maxY + Yn : r.minY - Yn,
					leavesPositiveSide: i
				};
			}
			let a = e.x > (t.x ?? 0), o = (n.y ?? 0) >= e.y;
			return {
				x: a ? r.maxX + Jn : r.minX - Jn,
				y: o ? r.maxY + Yn : r.minY - Yn,
		...
		if (A.inside && A.obstacle) {
			...
			if (w) {
				let t = D(s, r, i, e, !0);
				v.x = t.x, v.y = t.y;
				let n = t.leavesPositiveSide ? Math.min(e.minY - 2, s.y + Zn) : Math.max(e.maxY + 2, s.y - Zn);
				O = [
					{
						x: s.x,
						y: n
				...
				let t = D(s, r, i, e, !1), n = t.leavesPositiveSide ? Math.min(e.minX - 2, s.x + Zn) : Math.max(e.maxX + 2, s.x - Zn);
				v.x = t.x, v.y = t.y, O = [
					{
						x: n,
						y: s.y
		...
		if (O.length === 0 && j.length === 0) {
			let e = Jn, r = Math.abs(v.x - b.x) < e, i = Math.abs(v.y - b.y) < e, a = te.get(`${t}:src`) !== void 0 || te.get(`${t}:dst`) !== void 0, o = (S.get(`${n.start ?? ""}:${p}:src`)?.length ?? 0) + (S.get(`${n.start ?? ""}:${p}:dst`)?.length ?? 0), c = (S.get(`${n.end ?? ""}:${m}:src`)?.length ?? 0) + (S.get(`${n.end ?? ""}:${m}:dst`)?.length ?? 0), l = o > 1 || c > 1, d = C.get(n.start ?? "") ?? 0, f = C.get(n.end ?? "") ?? 0, g = o > 1 && ne(n.start, p) || c > 1 && ne(n.end, m);
		...
		let M = Math.min(v.x, b.x) - 50, N = Math.max(v.x, b.x) + 50, F = Math.min(v.y, b.y) - 50, I = Math.max(v.y, b.y) + 50;
		for (let e of d) {
			let t = Math.min(v.x, b.x), n = Math.max(v.x, b.x), r = Math.min(v.y, b.y), i = Math.max(v.y, b.y);
			e.minX < n && e.maxX > t && e.minY < i && e.maxY > r && (M = Math.min(M, e.minX - Xn), N = Math.max(N, e.maxX + Xn), F = Math.min(F, e.minY - Xn), I = Math.max(I, e.maxY + Xn));
		}
		for (let e of d) {
			if (e.maxX < M || e.minX > N || e.maxY < F || e.minY > I) continue;
			let t = Jn;
			f("horizontal", e.minY - t, M, N), f("horizontal", e.maxY + t, M, N);
			let n = Yn;
			f("vertical", e.minX - n, F, I), f("vertical", e.maxX + n, F, I);
		}
		f("horizontal", v.y, M, N), f("horizontal", b.y, M, N);
		let L = c.filter((e) => e.orientation === "horizontal" && e.coord >= F && e.coord <= I), R = c.filter((e) => e.orientation === "vertical" && e.coord >= M && e.coord <= N), z = /* @__PURE__ */ e((e, t) => `${e.toFixed(1)},${t.toFixed(1)}`, "getKey"), B = z(v.x, v.y), re = z(b.x, b.y), V = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), ie = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Set(), U = [];
		V.set(B, 0), ie.set(B, "n"), U.push({
			key: B,
			f: Math.hypot(b.x - v.x, b.y - v.y),
			pt: v
		}), ae.add(B);
		let W = [], G = /* @__PURE__ */ e((e, t) => x(e, t, n.start, n.end), "checkSegmentBlocked"), oe = {
			x: b.x,
			y: v.y
		}, se = G(v, oe), K = G(oe, b), ce = se || K, le = {
			x: v.x,
			y: b.y
		}, ue = G(v, le), de = G(le, b);
		if (ce ? ue || de || (W = Math.abs(v.x - b.x) < $ ? [v, b] : [
			v,
			le,
			b
		]) : W = Math.abs(v.y - b.y) < $ || Math.abs(v.x - b.x) < $ ? [v, b] : [
			v,
			oe,
			b
		], W.length === 0) for (; U.length > 0;) {
			U.sort((e, t) => e.f - t.f);
			let e = U.shift();
			if (ae.delete(e.key), e.key === re) {
				...
				for (W = [t]; H.has(e);) {
					let n = H.get(e);
					W.unshift(n), t = n, e = z(n.x, n.y);
				}
				break;
			}
			let r = e.pt.x, i = e.pt.y, a = R.sort((e, t) => e.coord - t.coord), o = a.findIndex((e) => Math.abs(e.coord - r) < 1), s = L.sort((e, t) => e.coord - t.coord), c = s.findIndex((e) => Math.abs(e.coord - i) < 1), l = [];
			o > 0 && l.push({
				x: a[o - 1].coord,
				y: i
			...
			for (let a of l) {
				...
				let u = z(a.x, a.y), f = Math.abs(a.x - r) + Math.abs(a.y - i), p = y(t, e.pt, a), m = 0, h = b.x - v.x, g = b.y - v.y, _ = a.x - r, x = a.y - i;
				(g > 10 && x < -5 || g < -10 && x > 5) && (m = Math.abs(x) * 100), (h > 10 && _ < -5 || h < -10 && _ > 5) && (m += Math.abs(_) * 50);
				let S = 0, C = ie.get(e.key) ?? "n", w = Math.abs(_) > $ ? "h" : "v";
				C !== "n" && C !== w && (S = 50);
				let T = f + p + m + S, E = (V.get(e.key) ?? Infinity) + T, D = Math.abs(b.x - a.x) + Math.abs(b.y - a.y);
				if (E < (V.get(u) ?? Infinity)) {
					if (H.set(u, e.pt), V.set(u, E), ie.set(u, w), !ae.has(u)) U.push({
						key: u,
						f: E + D,
						pt: a
					}), ae.add(u);
					else {
						let e = U.findIndex((e) => e.key === u);
						e !== -1 && (U[e].f = E + D);
					}
				}
			}
		...
		if (W.length === 0 && (W = [
			...
			let t = W[0], n = W[W.length - 1], r = Math.min(t.x, n.x), i = Math.max(t.x, n.x), a = Math.min(t.y, n.y), o = Math.max(t.y, n.y);
			for (let e of W) r = Math.min(r, e.x), i = Math.max(i, e.x), a = Math.min(a, e.y), o = Math.max(o, e.y);
			...
			let u = /* @__PURE__ */ e((e) => {
				...
				return n.y;
			}, "findBestReturnY"), f = /* @__PURE__ */ e((e) => {
				let r = u(e), i = {
					x: e,
					y: t.y
				}, a = {
					...
					x: n.x,
					y: r
				}, s = G(t, i), c = G(i, a), l = G(a, o), d = r !== n.y && G(o, n);
				return !s && !c && !l && !d ? Math.abs(r - n.y) < $ ? [
					t,
					i,
					a,
					n
		...
		if (q.length >= 3) {
			let e = q[q.length - 1], t = q[q.length - 2], n = q[q.length - 3], r = Math.abs(n.y - t.y) < $ && Math.abs(t.y - e.y) < $, i = Math.abs(n.x - t.x) < $ && Math.abs(t.x - e.x) < $;
			if (r) {
		...
		for (let e = 1; e < q.length - 1; e++) {
			...
			let t = fe[fe.length - 1], n = q[e], r = q[e + 1];
		...
		fe.push(q[q.length - 1]);
		for (let e = 0; e < fe.length - 1; e++) {
			let n = fe[e], r = fe[e + 1], i = Math.abs(n.x - r.x) < $ ? "vertical" : "horizontal", a = i === "vertical" ? n.x : n.y, o = i === "vertical" ? Math.min(n.y, r.y) : Math.min(n.x, r.x), s = i === "vertical" ? Math.max(n.y, r.y) : Math.max(n.x, r.x), c = f(i, a, o, s), l = {
				edgeIndex: t,
				segmentIndex: e,
				orientation: i,
				pipe: c,
	...
	let F = /* @__PURE__ */ e((e, t) => e.from < t.to && t.from < e.to, "segmentsOverlap"), I = /* @__PURE__ */ e((e, t, n, r) => {
		let i = !r.segments.some((n) => (n.edgeIndex !== t.edgeIndex || n.segmentIndex !== t.segmentIndex) && F(n, e)), a = !n.segments.some((n) => (n.edgeIndex !== e.edgeIndex || n.segmentIndex !== e.segmentIndex) && F(n, t));
	...
	}, "trySwapSegmentsAcrossTracks"), L = /* @__PURE__ */ e((e) => {
	...
	}, "createNewTrack"), R = /* @__PURE__ */ e((e, t) => {
	...
	}, "moveSegmentToTrack"), z = /* @__PURE__ */ e((e, t) => {
		let n = g[e.edgeIndex];
		for (let r of n) {
			let n = h[r];
			n.pipe === e.pipe && R(n, t);
		}
	}, "moveSegmentChainToTrack"), B = /* @__PURE__ */ e((e) => {
		let t = g[e.edgeIndex], n = t.indexOf(h.indexOf(e)), r = [];
		return n > 0 && r.push(h[t[n - 1]]), n < t.length - 1 && r.push(h[t[n + 1]]), r;
	}, "getAdjacentSegmentsAlongEdge"), re = /* @__PURE__ */ e((e, t) => {
	...
	}, "haveAnyCrossing"), V = /* @__PURE__ */ e((e, t) => {
		for (let n of e.tracks) if (!n.segments.some((e) => (e.edgeIndex !== t.edgeIndex || e.segmentIndex !== t.segmentIndex) && F(e, t))) return n.index;
		return -1;
	}, "findAvailableTrack"), H = /* @__PURE__ */ e((e, t) => {
		if (e.trackIndex === t.trackIndex) return F(e, t);
		let n = B(e), r = B(t);
		return n.some((e) => r.some((t) => re(e, t)));
	}, "segmentsConflict"), ie = /* @__PURE__ */ e((e, t, n) => {
		if (I(e, t, e.pipe.tracks[e.trackIndex], t.pipe.tracks[t.trackIndex])) return;
		let r = V(e.pipe, t);
		n(t, r === -1 ? L(e.pipe) : r);
	}, "resolveTrackConflict"), ae = /* @__PURE__ */ e((e) => {
		let t = 0;
		for (let n = 0; n < e.length; n++) for (let r = n + 1; r < e.length; r++) {
			let i = e[n], a = e[r];
			i.pipe === a.pipe && H(i, a) && (t++, ie(i, a, z));
		}
		return t;
	}, "resolveHandleConflicts"), U = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ e((e) => {
		...
		return t;
	}, "fixSourceHandleCrossings"), oe = /* @__PURE__ */ e(() => {
		let t = 0, n = /* @__PURE__ */ new Map();
		...
		return t;
	}, "fixTargetHandleCrossings"), se = /* @__PURE__ */ e(() => {
		let e = 0;
		for (let t of c) {
			let n = [];
		...
		return e;
	}, "fixPipeCrossings"), K = 0;
	for (; K < 10;) {
		let e = 0;
		if (e += G(), e += oe(), e += se(), e === 0) break;
		K++;
	}
		...
		for (let n of r) {
			...
			let c = /* @__PURE__ */ e((e, r) => {
				n.filter((t) => t.trackIndex === e).forEach((e) => {
					...
					ce.set(`${e.edgeIndex}-${e.segmentIndex}`, n);
				});
			}, "assignCoord"), l = 0;
			for (let e of a) l++, c(e, t.coord - l * Qn);
			...
			for (let e of s) {
				if (u === 0) c(e, t.coord);
					...
					c(e, t.coord + n * r * Qn * .5);
				}
				u++;
			}
			let d = 0;
			for (let e of o) d++, c(e, t.coord + d * Qn);
		}
	}
	for (let [e, t] of a.entries()) {
		let n = g[e] ?? [];
		if (n.length === 0) continue;
		let r = [], { pSrcPort: i, pDstPort: a } = P(e, o.get(t.start), o.get(t.end)), s = n.map((e) => {
			let t = h[e], n = ce.get(`${t.edgeIndex}-${t.segmentIndex}`) ?? t.pipe.coord;
			return {
		...
		for (let e = 0; e < s.length; e++) {
			...
			if (Math.abs(a - t.coord) > $ && r.push(tr(t, i)), c && o.orient === t.orient) {
				if (Math.abs(t.coord - o.coord) > $) {
					let e = t.orient === "vertical" ? (i + o.from) / 2 : er(t, o);
					r.push(tr(t, e), tr(o, e));
				} else (e === 0 || e === s.length - 2) && r.push(tr(t, er(t, o)));
			} else if (c) r.push(tr(t, o.coord));
			else {
		...
		let c = r[r.length - 1];
		(Math.abs(c.x - a.x) > $ || Math.abs(c.y - a.y) > $) && r.push(a);
		let l = [];
	...
	for (let e of t.edges) {
		...
		i && (t[0] = le(t[0], i)), a && (t[t.length - 1] = le(t[t.length - 1], a));
	}
	return t;
}
e(nr, "routeEdgesOrthogonal");
function rr(e) {
	return e.direction ?? "TB";
}
e(rr, "getSwimlaneDirection");
function ir(e) {
	let t = A(e), n = e.config.flowchart?.nodeSpacing ?? 40, r = e.config.flowchart?.rankSpacing ?? 100, i = e.config.swimlane?.ignoreCrossLaneEdges ?? !0, a = e.config.swimlane?.optimizeRanksByCrossings ?? !0, o = e.config.swimlane?.automaticLaneOrdering ?? !1, s = rr(e), { ordered: c, coordinates: l } = Kn(t, {
		nodeGap: n,
		layerGap: r,
	...
	return It(e, s), Ft(e), s;
}
e(ir, "runSwimlaneLayoutCore");
function ar(e) {
	k(e);
	let t = te(e);
	e.nodes = t.nodes, e.edges = t.edges;
}
e(ar, "prepareSwimlaneLayout");
var or = r({
	prepareLayout: ar,
	runLayoutCore: ir,
	afterPaint: C
