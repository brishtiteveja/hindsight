...
import { H as i, K as a, U as o, Y as s, a as c, b as l, f as u, s as d, v as f, w as p, x as m, y as h } from "./chunk-DU6HZSFF-Bv553b-w.mjs";
import { g, i as _, m as v } from "./chunk-75Z2AOVW-DlpwHa_5.mjs";
import { t as y } from "./chunk-JWPE2WC7-DIKiF5cm.mjs";
import { t as b } from "./chunk-2Q5K7J3B-BDbtAK2d.mjs";
...
var te = /* @__PURE__ */ t(function(e) {
...
}, "setDirection"), ne = /* @__PURE__ */ t(function(e) {
	n.debug("options str", e), e = e?.trim(), e = e || "{}";
	try {
		C.records.options = JSON.parse(e);
	} catch (e) {
		n.error("error while parsing gitGraph options", e.message);
	}
}, "setOptions"), re = /* @__PURE__ */ t(function() {
	return C.records.options;
}, "getOptions"), ie = /* @__PURE__ */ t(function(e) {
	let t = e.msg, r = e.id, i = e.type, a = e.tags;
	n.info("commit", t, r, i, a), n.debug("Entering commit:", t, r, i, a);
	let o = S();
	r = d.sanitizeText(r, o), t = d.sanitizeText(t, o), a = a?.map((e) => d.sanitizeText(e, o));
	let s = {
	...
	C.records.head = s, n.info("main branch", o.mainBranchName), C.records.commits.has(s.id) && n.warn(`Commit ID ${s.id} already exists`), C.records.commits.set(s.id, s), C.records.branches.set(C.records.currBranch, s.id), n.debug("in pushCommit " + s.id);
}, "commit"), ae = /* @__PURE__ */ t(function(e) {
	let t = e.name, r = e.order;
	if (t = d.sanitizeText(t, S()), C.records.branches.has(t)) throw Error(`Trying to create an existing branch. (Help: Either use a new name if you want create a new branch or try using "checkout ${t}")`);
	C.records.branches.set(t, C.records.head == null ? null : C.records.head.id), C.records.branchConfig.set(t, {
		name: t,
		order: r
	}), E(t), n.debug("in createBranch");
}, "branch"), oe = /* @__PURE__ */ t((e) => {
	let t = e.branch, r = e.id, i = e.type, a = e.tags, o = S();
	t = d.sanitizeText(t, o), r && (r = d.sanitizeText(r, o));
	let s = C.records.branches.get(C.records.currBranch), c = C.records.branches.get(t), l = s ? C.records.commits.get(s) : void 0, u = c ? C.records.commits.get(c) : void 0;
	...
	if (r && C.records.commits.has(r)) {
		...
		throw e.hash = {
			text: `merge ${t} ${r} ${i} ${a?.join(" ")}`,
			token: `merge ${t} ${r} ${i} ${a?.join(" ")}`,
			expected: [`merge ${t} ${r}_UNIQUE ${i} ${a?.join(" ")}`]
		}, e;
	}
	let f = c || "", p = {
		id: r || `${C.records.seq}-${w()}`,
	...
	C.records.head = p, C.records.commits.set(p.id, p), C.records.branches.set(C.records.currBranch, p.id), n.debug(C.records.branches), n.debug("in mergeBranch");
}, "merge"), se = /* @__PURE__ */ t(function(e) {
	let t = e.id, r = e.targetId, i = e.tags, a = e.parent;
	n.debug("Entering cherryPick:", t, r, i);
	let o = S();
	if (t = d.sanitizeText(t, o), r = d.sanitizeText(r, o), i = i?.map((e) => d.sanitizeText(e, o)), a = d.sanitizeText(a, o), !t || !C.records.commits.has(t)) {
	...
	if (!r || !C.records.commits.has(r)) {
		...
		C.records.head = l, C.records.commits.set(l.id, l), C.records.branches.set(C.records.currBranch, l.id), n.debug(C.records.branches), n.debug("in cherryPick");
	}
}, "cherryPick"), E = /* @__PURE__ */ t(function(e) {
	if (e = d.sanitizeText(e, S()), C.records.branches.has(e)) {
...
function O(e) {
	...
	for (let e in C.records.branches) C.records.branches.get(e) === t.id && i.push(e);
	if (n.debug(i.join(" ")), t.parents && t.parents.length == 2 && t.parents[0] && t.parents[1]) {
...
var ce = /* @__PURE__ */ t(function() {
	...
	i ? i(e) : n.error(`Unknown statement type: ${e.$type}`);
	...
	"redux-color",
	"redux-dark-color"
]), L = 12, R = /* @__PURE__ */ new Set(["redux-color", "redux-dark-color"]), xe = /* @__PURE__ */ new Set([
	"dark",
	"redux-dark",
	"redux-dark-color",
	"neo-dark"
]), z = /* @__PURE__ */ t((e, t, n = !1) => n && e > 0 ? (e - 1) % (t - 1) + 1 : e % t, "calcColorIndex"), B = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), H = 30, U = /* @__PURE__ */ new Map(), W = [], G = 0, K = "LR", Se = /* @__PURE__ */ t(() => {
...
}, "findClosestParent"), Ce = /* @__PURE__ */ t((e) => {
	let t = "", n = Infinity;
	return e.forEach((e) => {
		let r = V.get(e).y;
		r <= n && (t = e, n = r);
	}), t || void 0;
}, "findClosestParentBT"), we = /* @__PURE__ */ t((e, t, n) => {
	let r = n, i = n, a = [];
	e.forEach((e) => {
		let n = t.get(e);
		if (!n) throw Error(`Commit not found for key ${e}`);
		n.parents.length ? (r = Ee(n), i = Math.max(r, i)) : a.push(n), De(n, r);
	}), r = i, a.forEach((e) => {
		Oe(e, r, n);
	}), e.forEach((e) => {
		let n = t.get(e);
...
}, "setParallelBTPos"), Te = /* @__PURE__ */ t((e) => {
	...
	return n;
}, "findClosestParentPos"), Ee = /* @__PURE__ */ t((e) => Te(e) + M, "calculateCommitPosition"), De = /* @__PURE__ */ t((e, t) => {
...
}, "setCommitPosition"), Oe = /* @__PURE__ */ t((e, t, n) => {
...
}, "setRootPosition"), ke = /* @__PURE__ */ t((e, t, n, r, i, a) => {
	let { theme: o } = m(), s = I.has(o ?? ""), c = R.has(o ?? ""), l = xe.has(o ?? "");
...
}, "drawCommitBullet"), Ae = /* @__PURE__ */ t((e, t, n, r, i) => {
	if (t.type !== x.CHERRY_PICK && (t.customId && t.type === x.MERGE || t.type !== x.MERGE) && i.showCommitLabel) {
		...
		if (c && (o.attr("x", n.posWithOffset - c.width / 2 - P).attr("y", n.y + 13.5).attr("width", c.width + 2 * P).attr("height", c.height + 2 * P), K === "TB" || K === "BT" ? (o.attr("x", n.x - (c.width + 4 * N + 5)).attr("y", n.y - 12), s.attr("x", n.x - (c.width + 4 * N)).attr("y", n.y + c.height - 12)) : s.attr("x", n.posWithOffset - c.width / 2), i.rotateCommitLabel)) {
			if (K === "TB" || K === "BT") s.attr("transform", "rotate(-45, " + n.x + ", " + n.y + ")"), o.attr("transform", "rotate(-45, " + n.x + ", " + n.y + ")");
			else {
...
}, "drawCommitLabel"), je = /* @__PURE__ */ t((e, t, n, r) => {
	if (t.tags.length > 0) {
		let i = 0, a = 0, o = 0, s = [];
...
}, "drawCommitTags"), Me = /* @__PURE__ */ t((e) => {
...
}, "getCommitClassType"), Ne = /* @__PURE__ */ t((e, t, n, r) => {
	let i = {
		x: 0,
		y: 0
	};
	...
	return 0;
}, "calculatePosition"), Pe = /* @__PURE__ */ t((e, t, n) => {
	...
	K === "BT" && (l && we(d, n, s), d = d.reverse()), d.forEach((e) => {
		let t = n.get(e);
		if (!t) throw Error(`Commit not found for key ${e}`);
		l && (s = Ne(t, K, s, V));
		let c = Pe(t, s, l);
		if (r) {
			let e = Me(t), n = t.customType ?? t.type, r = B.get(t.branch)?.index ?? 0;
			ke(a, t, c, e, r, n), Ae(o, t, c, s, i), je(o, t, c, s);
		}
...
}, "drawCommits"), Fe = /* @__PURE__ */ t((e, n, r, i, a) => {
...
}, "findLane"), Ie = /* @__PURE__ */ t((e, t, n, r) => {
	let { theme: i } = m(), a = R.has(i ?? ""), o = V.get(t.id), s = V.get(n.id);
	if (o === void 0 || s === void 0) throw Error(`Commit positions not found for commits ${t.id} and ${n.id}`);
	let c = Fe(t, n, o, s, r), l = "", u = "", d = 0, f = 0, p = B.get(n.branch)?.index;
	n.type === x.MERGE && t.id !== n.parents[0] && (p = B.get(t.branch)?.index);
	let h;
	if (c) {
		l = "A 10 10, 0, 0, 0,", u = "A 10 10, 0, 0, 1,", d = 10, f = 10;
		let e = o.y < s.y ? X(o.y, s.y) : X(s.y, o.y), n = o.x < s.x ? X(o.x, s.x) : X(s.x, o.x);
		K === "TB" ? o.x < s.x ? h = `M ${o.x} ${o.y} L ${n - d} ${o.y} ${u} ${n} ${o.y + f} L ${n} ${s.y - d} ${l} ${n + f} ${s.y} L ${s.x} ${s.y}` : (p = B.get(t.branch)?.index, h = `M ${o.x} ${o.y} L ${n + d} ${o.y} ${l} ${n} ${o.y + f} L ${n} ${s.y - d} ${u} ${n - f} ${s.y} L ${s.x} ${s.y}`) : K === "BT" ? o.x < s.x ? h = `M ${o.x} ${o.y} L ${n - d} ${o.y} ${l} ${n} ${o.y - f} L ${n} ${s.y + d} ${u} ${n + f} ${s.y} L ${s.x} ${s.y}` : (p = B.get(t.branch)?.index, h = `M ${o.x} ${o.y} L ${n + d} ${o.y} ${u} ${n} ${o.y - f} L ${n} ${s.y + d} ${l} ${n - f} ${s.y} L ${s.x} ${s.y}`) : o.y < s.y ? h = `M ${o.x} ${o.y} L ${o.x} ${e - d} ${l} ${o.x + f} ${e} L ${s.x - d} ${e} ${u} ${s.x} ${e + f} L ${s.x} ${s.y}` : (p = B.get(t.branch)?.index, h = `M ${o.x} ${o.y} L ${o.x} ${e + d} ${u} ${o.x + f} ${e} L ${s.x - d} ${e} ${l} ${s.x} ${e - f} L ${s.x} ${s.y}`);
	...
	e.append("path").attr("d", h).attr("class", "arrow arrow" + z(p, F, a));
}, "drawArrow"), Le = /* @__PURE__ */ t((e, t) => {
...
}, "drawArrows"), Re = /* @__PURE__ */ t((e, t, n, r) => {
	let { look: i, theme: a, themeVariables: o } = m(), { dropShadow: s, THEME_COLOR_LIMIT: c } = o, l = I.has(a ?? ""), u = R.has(a ?? ""), d = e.append("g");
	t.forEach((e, t) => {
...
}, "drawBranches"), ze = /* @__PURE__ */ t(function(e, t, n, r, i) {
	return B.set(e, {
		pos: t,
		index: n
	}), t += 50 + (i ? 40 : 0) + (K === "TB" || K === "BT" ? r.width / 2 : 0), t;
}, "setBranchPosition"), Be = { draw: /* @__PURE__ */ t(function(e, t, i, a) {
	Se(), n.debug("in gitgraph renderer", e + "\n", "id:", t, i);
	let o = a.db;
	if (!o.getConfig) {
		n.error("getConfig method is not available on db");
		return;
	}
	let c = o.getConfig(), l = c.rotateCommitLabel ?? !1;
	U = o.getCommits();
	let u = o.getBranchesAsObjArray();
	K = o.getDirection();
	let d = r(`[id="${t}"]`), { look: f, theme: p, themeVariables: h } = m(), { useGradient: _, gradientStart: v, gradientStop: y, filterColor: b } = h;
	if (_) {
	...
	f === "neo" && I.has(p ?? "") && d.append("defs").append("filter").attr("id", t + "-drop-shadow").attr("height", "130%").attr("width", "130%").append("feDropShadow").attr("dx", "4").attr("dy", "4").attr("stdDeviation", 0).attr("flood-opacity", "0.06").attr("flood-color", b);
	let x = 0;
	u.forEach((e, t) => {
	...
	}), Y(d, U, !1, c), c.showBranches && Re(d, u, c, t), Le(d, U), Y(d, U, !0, c), g.insertTitle(d, "gitTitleText", c.titleTopMargin ?? 0, o.getDiagramTitle()), s(void 0, d, c.diagramPadding, c.useMaxWidth);
}, "draw") }, Z = 8, Q = /* @__PURE__ */ new Set([
	"redux",
	"redux-dark",
	"redux-color",
	"redux-dark-color"
]), Ve = /* @__PURE__ */ new Set(["redux-color", "redux-dark-color"]), $ = /* @__PURE__ */ new Set(["neo", "neo-dark"]), He = /* @__PURE__ */ new Set([
	"dark",
	"redux-dark",
	"redux-dark-color",
	"neo-dark"
	...
	let { theme: t, themeVariables: n } = l(), { borderColorArray: r } = n, i = Q.has(t);
	if ($.has(t)) {
		let t = "";
