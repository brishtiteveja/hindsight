...
import { H as t, K as n, U as r, a as i, c as a, s as o, v as s, w as c, x as l, y as u } from "./chunk-DU6HZSFF-Bv553b-w.mjs";
import { p as d } from "./chunk-75Z2AOVW-DlpwHa_5.mjs";
import { t as f } from "./rough.esm-DftXE3FS.mjs";
import { f as p } from "./mermaid.core-DuekSTCI.mjs";
...
var g = h, _ = (m = class {
	...
	let i = r.db.getRoot();
	if (!i) return;
	let a = l(), { look: o, handDrawnSeed: s, themeVariables: c } = a, u = d(a.fontSize)[0] ?? v, m = o === "handDrawn", h = i.children ?? [], g = a.ishikawa?.diagramPadding ?? 20, _ = a.ishikawa?.useMaxWidth ?? !1, b = p(t), x = b.append("g").attr("class", "ishikawa"), S = m ? f.svg(b.node()) : void 0, C = S ? {
		roughSvg: S,
		seed: s ?? 0,
		lineColor: c?.lineColor ?? "#333",
		fillColor: c?.mainBkg ?? "#fff"
	} : void 0, w = `ishikawa-arrow-${t}`;
	m || x.append("defs").append("marker").attr("id", w).attr("viewBox", "0 0 10 10").attr("refX", 0).attr("refY", 5).attr("markerWidth", 6).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M 10 0 L 0 5 L 10 10 Z").attr("class", "ishikawa-arrow");
	let T = 0, D = y, A = m ? void 0 : R(x, T, D, T, D, "ishikawa-spine");
	if (k(x, T, D, i.text, u, C), !h.length) {
		m && R(x, T, D, T, D, "ishikawa-spine", C), E(b, g, _);
		return;
	}
	T -= 20;
	let j = h.filter((e, t) => t % 2 == 0), M = h.filter((e, t) => t % 2 == 1), P = O(j), F = O(M), I = P.total + F.total, L = y, z = y;
	if (I > 0) {
		let e = y * 2, t = y * .3;
		L = Math.max(t, e * (P.total / I)), z = Math.max(t, e * (F.total / I));
	}
	let B = u * 2;
	L = Math.max(L, P.max * B), z = Math.max(z, F.max * B), D = Math.max(L, y), A && A.attr("y1", D).attr("y2", D), x.select(".ishikawa-head-group").attr("transform", `translate(0,${D})`);
	...
	for (let e = 0; e < V; e++) {
		...
		T = t.selectAll("text").nodes().reduce((e, t) => Math.min(e, t.getBBox().x), Infinity);
	}
	if (m) R(x, T, D, 0, D, "ishikawa-spine", C);
	else {
		A.attr("x1", T);
		let e = `url(#${w})`;
		x.selectAll("line.ishikawa-branch, line.ishikawa-sub-branch").attr("marker-start", e);
	}
	E(b, g, _);
}, "draw"), O = /* @__PURE__ */ e((t) => {
	...
	let c = t.children ?? [], l = a * (c.length ? 1 : .2), u = -w * l, d = T * l * i, f = n + u, p = r + d;
	if (R(e, n, r, f, p, "ishikawa-branch", s), s && M(e, n, r, n - f, r - p, s), j(e, t.text, f, p, i, o, s), !c.length) return;
	let { entries: m, yOrder: h } = A(c, i), g = m.length, _ = Array(g);
	...
	let y = -w, C = T * i, E = i < 0 ? "ishikawa-label up" : "ishikawa-label down";
	for (let [t, n] of m.entries()) {
		let r = _[t], i = v.get(n.parentIndex), a = e.append("g").attr("class", "ishikawa-sub-group"), c = 0, l = 0, u = 0;
		if (n.depth % 2 == 0) {
			let e = i.y1 - i.y0;
			...
			c = L(i.x0, i.x1, (i.childCount - e) / (i.childCount + 1)), l = i.y0, u = c + y * ((r - l) / C), R(a, c, l, u, r, "ishikawa-sub-branch", s), s && M(a, c, l, c - u, l - r, s), I(a, n.text, u, r, E, "end", o);
		}
		n.childCount > 0 && v.set(t, {
			x0: c,
			y0: l,
			x1: u,
			y1: r,
