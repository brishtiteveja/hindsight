import { t as e } from "./rolldown-runtime-DArdT4gl.mjs";
//#region node_modules/@jetbrains/websandbox/dist/websandbox.js
var t = /* @__PURE__ */ e(((e, t) => {
	(function(n, r) {
		typeof e == "object" && typeof t == "object" ? t.exports = r() : typeof define == "function" && define.amd ? define([], r) : typeof e == "object" ? e.Websandbox = r() : n.Websandbox = r();
	})(self, () => (() => {
		var e = { 524(e) {
		...
		return (() => {
			...
			function i(t) {
				...
				for (let i = 0; i < t.length; i++) {
					let a = t[i];
					e.test(a) && t[i - 1] !== "\\" && (n.push(t.substring(r, i)), r = i + 1);
				}
				return n.push(t.substring(r, t.length)), n.filter((e) => !!e).map((e) => e.replace(/\\/g, ""));
			}
			function a(e) {
			...
			function o(e, t) {
				let n = i(t);
				return a(n) ? null : n.reduce((e, t) => e ? e[t] : null, e);
			}
			let s = "message", c = "response", l = "set-interface", u = "service-message", d = !!window.MSInputMethodContext && !!document.documentMode, f = { allowedSenderOrigin: "null" };
			class p {
				constructor(e, t, n = {}) {
					this.remote = {}, this.serviceMethods = {}, this.localApi = {}, this.callbacks = {}, this._resolveRemoteMethodsPromise = null, this.options = Object.assign(Object.assign({}, f), n), this.incrementalID = Math.floor(Math.random() * 1e5), this.postMessage = e, this.remoteMethodsWaitPromise = new Promise((e) => {
						this._resolveRemoteMethodsPromise = e;
					}), t((e) => this.onMessageListener(e));
				}
				onMessageListener(e) {
					let t = e.data, { allowedSenderOrigin: n } = this.options;
					(!n || e.origin === n || d) && (t.type === c ? this.popCallback(t.callId, t.success, t.result) : t.type === s ? this.callLocalApi(t.methodName, t.arguments).then((e) => this.responseOtherSide(t.callId, e)).catch((e) => this.responseOtherSide(t.callId, e, !1)) : t.type === l ? (this.setInterface(t.apiMethods), this.responseOtherSide(t.callId)) : t.type === u && this.callLocalServiceMethod(t.methodName, t.arguments).then((e) => this.responseOtherSide(t.callId, e)).catch((e) => this.responseOtherSide(t.callId, e, !1)));
				}
				postMessageToOtherSide(e) {
					this.postMessage(e, "*");
				}
				setInterface(e) {
					var t;
					this.remote = {}, e.forEach((e) => {
						let t = i(e);
						if (a(t)) return;
						let n = this.remote;
						for (let e = 0; e < t.length - 1; e++) {
							let r = t[e];
							(!n[r] || typeof n[r] != "object") && (n[r] = {}), n = n[r];
						}
						n[t[t.length - 1]] = this.createMethodWrapper(e);
					}), (t = this._resolveRemoteMethodsPromise) == null || t.call(this);
				}
				getMethodsFromInterface(e) {
					return Object.keys(e).reduce((t, n) => (typeof e[n] == "object" ? t.push(...this.getMethodsFromInterface(e[n]).map((e) => `${n}.${e}`)) : t.push(n), t), []);
				}
				setLocalApi(e) {
					return new Promise((t, n) => {
						let r = this.registerCallback(t, n);
						this.postMessageToOtherSide({
							callId: r,
							apiMethods: this.getMethodsFromInterface(e),
							type: l
						});
					}).then(() => this.localApi = e);
				}
				setServiceMethods(e) {
					this.serviceMethods = e;
				}
				callLocalApi(e, t) {
					let n = o(this.localApi, e);
					if (!n) throw Error(`Local method "${e}" is not registered`);
					return Promise.resolve(n.call(this, ...t));
				}
				callLocalServiceMethod(e, t) {
					let n = o(this.serviceMethods, e);
					if (!n) throw Error(`Service method ${e} is not registered`);
					return Promise.resolve(n.call(this, ...t));
				}
				createMethodWrapper(e) {
					return (...t) => this.callRemoteMethod(e, ...t);
				}
				callRemoteMethod(e, ...t) {
					return new Promise((n, r) => {
						let i = this.registerCallback(n, r);
						this.postMessageToOtherSide({
							callId: i,
							methodName: e,
							type: s,
				...
				responseOtherSide(e, t, n = !0) {
					t instanceof Error && (t = [...Object.keys(t), "message"].reduce((e, n) => (e[n] = t[n], e), {}));
					let r = () => this.postMessage({
						callId: e,
						type: c,
						success: n,
						result: t
				...
				registerCallback(e, t) {
					let n = (++this.incrementalID).toString();
				...
				popCallback(e, t, n) {
					...
					r && (t ? r.successCallback(n) : r.failureCallback(n), delete this.callbacks[e]);
				}
			}
			let m = p;
			var h = n(524), g = /*#__PURE__*/ n.n(h);
			let _ = {
				frameContainer: "body",
			...
			class v {
				...
				constructor(e, t) {
					this.connection = null, this.removeMessageListener = () => {}, this.validateOptions(t), this.options = Object.assign(Object.assign({}, _), t), this.iframe = this.createIframe(), this.promise = new Promise((t) => {
						this.connection = new m(this.iframe.contentWindow.postMessage.bind(this.iframe.contentWindow), (e) => {
							let t = (t) => {
								if (t.source === this.iframe.contentWindow) return e(t);
							};
							window.addEventListener("message", t), this.removeMessageListener = () => window.removeEventListener("message", t);
						}, { allowedSenderOrigin: "null" }), this.connection.setServiceMethods({ iframeInitialized: () => this.connection.setLocalApi(e).then(() => t(this)) });
					});
				}
				validateOptions(e) {
					if (e.frameSrc && (e.frameContent || e.initialStyles || e.baseUrl || e.codeToRunBeforeInit)) throw Error("You can not set both \"frameSrc\" and any of frameContent,initialStyles,baseUrl,codeToRunBeforeInit options");
					if ("frameContent" in e && !e.frameContent?.includes("<head>")) throw Error("Websandbox: iFrame content must have \"<head>\" tag.");
				}
				_prepareFrameContent(e) {
					let t = e.frameContent ?? "";
					return e.codeToRunBeforeInit && (t = t.replace("<head>", `<head><script>${e.codeToRunBeforeInit}<\/script>`) ?? ""), t = t.replace("<head>", `<head><script>${g()}<\/script>`) ?? "", e.initialStyles && (t = t.replace("</head>", `<style>${e.initialStyles}</style></head>`)), e.baseUrl && (t = t.replace("<head>", `<head><base target="_parent" href="${e.baseUrl}"/>`)), t;
				}
				createIframe() {
					let e = this.options.frameContainer, t = typeof e == "string" ? document.querySelector(e) : e;
					if (!t) throw Error("Websandbox: Cannot find container for sandbox " + t);
					let n = document.createElement("iframe");
					if (n.sandbox = `allow-scripts ${this.options.sandboxAdditionalAttributes}`, n.allow = `${this.options.allowAdditionalAttributes}`, n.className = this.options.frameClassName ?? "", this.options.allowFullScreen && (n.allowFullscreen = !0), this.options.frameSrc) return n.src = this.options.frameSrc, t.appendChild(n), n;
					let r = this._prepareFrameContent(this.options), i = this.options.trustedTypesPolicy?.createHTML(r) ?? r;
					return n.setAttribute("srcdoc", i), t.appendChild(n), n;
				}
				destroy() {
				...
				_runFunction(e) {
					return this._runCode(`(${e.toString()})()`);
				}
				run(e) {
