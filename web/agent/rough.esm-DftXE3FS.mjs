...
function n(n, r, i, a = 1) {
	let o = i, s = Math.max(r, .1), c = n[0] && n[0][0] && typeof n[0][0] == "number" ? [n] : n, l = [0, 0];
	if (o) for (let t of c) e(t, l, o);
	let u = function(e, n, r) {
		let i = [];
		for (let n of e) {
			let e = [...n];
			t(e[0], e[e.length - 1]) || e.push([e[0][0], e[0][1]]), e.length > 2 && i.push(e);
		}
		let a = [];
		n = Math.max(n, .1);
		let o = [];
		...
		if (o.sort(((e, t) => e.ymin < t.ymin ? -1 : e.ymin > t.ymin ? 1 : e.x < t.x ? -1 : e.x > t.x ? 1 : e.ymax === t.ymax ? 0 : (e.ymax - t.ymax) / Math.abs(e.ymax - t.ymax))), !o.length) return a;
		let s = [], c = o[0].ymin, l = 0;
		for (; s.length || o.length;) {
			...
			if (s = s.filter(((e) => !(e.edge.ymax <= c))), s.sort(((e, t) => e.edge.x === t.edge.x ? 0 : (e.edge.x - t.edge.x) / Math.abs(e.edge.x - t.edge.x))), (r !== 1 || l % n == 0) && s.length > 1) for (let e = 0; e < s.length; e += 2) {
				let t = e + 1;
				if (t >= s.length) break;
				let n = s[e].edge, r = s[t].edge;
				a.push([[Math.round(n.x), c], [Math.round(r.x), c]]);
			}
			c += r, s.forEach(((e) => {
...
function r(e, t) {
	...
	i < 0 && (i = 4 * t.strokeWidth), i = Math.round(Math.max(i, .1));
	let a = 1;
	return t.roughness >= 1 && (t.randomizer?.next() || Math.random()) > .7 && (a = i), n(e, i, r, a || 1);
}
var i = class {
	constructor(e) {
		this.helper = e;
...
var o = class extends i {
	fillPolygons(e, t) {
		...
		let i = r(e, Object.assign({}, t, { hachureGap: n })), o = Math.PI / 180 * t.hachureAngle, s = [], c = .5 * n * Math.cos(o), l = .5 * n * Math.sin(o);
		for (let [e, t] of i) a([e, t]) && s.push([[e[0] - c, e[1] + l], [...t]], [[e[0] + c, e[1] - l], [...t]]);
		return {
			type: "fillSketch",
			ops: this.renderLines(s, t)
		};
	}
}, s = class extends i {
	fillPolygons(e, t) {
	...
	dotsOnLines(e, t) {
		...
		let o = r / 4;
		for (let s of e) {
			let e = a(s), c = e / r, l = Math.ceil(c) - 1, u = e - l * r, d = (s[0][0] + s[1][0]) / 2 - r / 4, f = Math.min(s[0][1], s[1][1]);
			for (let e = 0; e < l; e++) {
				let a = f + u + e * r, s = d - o + 2 * Math.random() * o, c = a - o + 2 * Math.random() * o, l = this.helper.ellipse(s, c, i, i, t);
				n.push(...l.ops);
			}
		}
		return {
	...
	dashedLine(e, t) {
		...
		return e.forEach(((e) => {
			let o = a(e), s = Math.floor(o / (n + r)), c = (o + r - s * (n + r)) / 2, l = e[0], u = e[1];
			l[0] > u[0] && (l = e[1], u = e[0]);
			let d = Math.atan((u[1] - l[1]) / (u[0] - l[0]));
			for (let e = 0; e < s; e++) {
				let a = e * (n + r), o = a + n, s = [l[0] + a * Math.cos(d) + c * Math.cos(d), l[1] + a * Math.sin(d) + c * Math.sin(d)], u = [l[0] + o * Math.cos(d) + c * Math.cos(d), l[1] + o * Math.sin(d) + c * Math.sin(d)];
	...
	fillPolygons(e, t) {
		let n = t.hachureGap < 0 ? 4 * t.strokeWidth : t.hachureGap, i = t.zigzagOffset < 0 ? n : t.zigzagOffset, a = r(e, t = Object.assign({}, t, { hachureGap: n + i }));
		return {
			type: "fillSketch",
			ops: this.zigzagLines(a, i, t)
		};
	}
	zigzagLines(e, t, n) {
		let r = [];
		return e.forEach(((e) => {
			let i = a(e), o = Math.round(i / (2 * t)), s = e[0], c = e[1];
			s[0] > c[0] && (s = e[1], c = e[0]);
			let l = Math.atan((c[1] - s[1]) / (c[0] - s[0]));
			for (let e = 0; e < o; e++) {
				let i = 2 * e * t, a = 2 * (e + 1) * t, o = Math.sqrt(2 * t ** 2), c = [s[0] + i * Math.cos(l), s[1] + i * Math.sin(l)], u = [s[0] + a * Math.cos(l), s[1] + a * Math.sin(l)], d = [c[0] + o * Math.cos(l + Math.PI / 4), c[1] + o * Math.sin(l + Math.PI / 4)];
				r.push(...this.helper.doubleLineOps(c[0], c[1], d[0], d[1], n), ...this.helper.doubleLineOps(d[0], d[1], u[0], u[1], n));
			}
		})), r;
	}
}, d = {}, f = class {
...
function v(e) {
	...
	for (; !_(a, h);) {
		let o = 0, s = [];
		if (r === "BOD") {
			if (a.text !== "M" && a.text !== "m") return v("M0,0" + e);
			i++, o = g[a.text], r = a.text;
		} else _(a, m) ? o = g[r] : (i++, o = g[a.text], r = a.text);
		if (!(i + o < n.length)) throw Error("Path data ended short");
		for (let e = i; e < i + o; e++) {
			let t = n[e];
			if (!_(t, m)) throw Error("Param not a number: " + r + "," + t.text);
			s[s.length] = +t.text;
		}
		if (typeof g[r] != "number") throw Error("Bad segment: " + r);
		{
			let e = {
				key: r,
				data: s
	...
	for (let { key: l, data: u } of e) {
		switch (l) {
			...
			case "A": {
				...
				}), r = c, i = l) : (r !== c || i !== l) && (S(r, i, c, l, e, n, a, o, s).forEach((function(e) {
					t.push({
						key: "C",
						data: e
					});
...
function S(e, t, n, r, i, a, o, s, c, l) {
	...
	if (Math.abs(_) > 120 * Math.PI / 180) {
		let e = m, t = n, s = r;
		m = c && m > p ? p + 120 * Math.PI / 180 * 1 : p + 120 * Math.PI / 180 * -1, f = S(n = h + i * Math.cos(m), r = g + a * Math.sin(m), t, s, i, a, o, 0, c, [
			m,
			e,
			h,
			g
	...
	_ = m - p;
	let v = Math.cos(p), y = Math.sin(p), b = Math.cos(m), C = Math.sin(m), w = Math.tan(_ / 4), T = 4 / 3 * i * w, E = 4 / 3 * a * w, D = [e, t], O = [e + T * y, t - E * v], k = [n + T * C, r - E * b], A = [n, r];
	if (O[0] = 2 * D[0] - O[0], O[1] = 2 * D[1] - O[1], l) return [
		O,
		k,
		A
	].concat(f);
	{
		...
		for (let t = 0; t < f.length; t += 3) {
			let n = x(f[t][0], f[t][1], u), r = x(f[t + 1][0], f[t + 1][1], u), i = x(f[t + 2][0], f[t + 2][1], u);
			e.push([
				n[0],
				n[1],
				r[0],
...
var C = {
	...
	ellipse: function(e, t, n, r, i) {
		return k(e, t, i, O(n, r, i)).opset;
	},
	doubleLineOps: function(e, t, n, r, i) {
		return R(e, t, n, r, i, !0);
	}
};
...
function T(e, t, n) {
	...
	if (r > 2) {
		let i = [];
		for (let t = 0; t < r - 1; t++) i.push(...R(e[t][0], e[t][1], e[t + 1][0], e[t + 1][1], n));
		return t && i.push(...R(e[r - 1][0], e[r - 1][1], e[0][0], e[0][1], n)), {
			type: "path",
			ops: i
		};
	}
	return r === 2 ? w(e[0][0], e[0][1], e[1][0], e[1][1], n) : {
		type: "path",
		ops: []
	};
}
...
function D(e, t) {
	if (e.length) {
		let n = typeof e[0][0] == "number" ? [e] : e, r = B(n[0], 1 * (1 + .2 * t.roughness), t), i = t.disableMultiStroke ? [] : B(n[0], 1.5 * (1 + .22 * t.roughness), P(t));
		for (let e = 1; e < n.length; e++) {
			let a = n[e];
			if (a.length) {
				let e = B(a, 1 * (1 + .2 * t.roughness), t), n = t.disableMultiStroke ? [] : B(a, 1.5 * (1 + .22 * t.roughness), P(t));
...
function k(e, t, n, r) {
	let [i, a] = H(r.increment, e, t, r.rx, r.ry, 1, r.increment * I(.1, I(.4, 1, n), n), n), o = V(i, null, n);
	if (!n.disableMultiStroke && n.roughness !== 0) {
		let [i] = H(r.increment, e, t, r.rx, r.ry, 1.5, 0, n), a = V(i, null, n);
		o = o.concat(a);
	}
	return {
		estimatedPoints: a,
...
function A(e, t, n, r, i, a, o, s, c) {
	...
	d += L(.01 * d, c), f += L(.01 * f, c);
	let p = i, m = a;
	for (; p < 0;) p += 2 * Math.PI, m += 2 * Math.PI;
	m - p > 2 * Math.PI && (p = 0, m = 2 * Math.PI);
	let h = 2 * Math.PI / c.curveStepCount, g = Math.min(h / 2, (m - p) / 2), _ = U(g, l, u, d, f, p, m, 1, c);
	if (!c.disableMultiStroke) {
		let e = U(g, l, u, d, f, p, m, 1.5, c);
		_.push(...e);
	}
	return o && (s ? _.push(...R(l, u, l + d * Math.cos(p), u + f * Math.sin(p), c), ...R(l, u, l + d * Math.cos(m), u + f * Math.sin(m), c)) : _.push({
		op: "lineTo",
		data: [l, u]
	}, {
		op: "lineTo",
...
function j(e, t) {
	let n = b(y(v(e))), r = [], i = [0, 0], a = [0, 0];
	for (let { key: e, data: o } of n) switch (e) {
		case "M":
			a = [o[0], o[1]], i = [o[0], o[1]];
			break;
		case "L":
			r.push(...R(a[0], a[1], o[0], o[1], t)), a = [o[0], o[1]];
			break;
		case "C": {
			let [e, n, i, s, c, l] = o;
		...
		case "Z": r.push(...R(a[0], a[1], i[0], i[1], t)), a = [i[0], i[1]];
	}
	return {
		type: "path",
		ops: r
...
function M(e, t) {
	...
	for (let r of e) if (r.length) {
		...
		if (i > 2) {
			n.push({
				op: "move",
				data: [r[0][0] + L(e, t), r[0][1] + L(e, t)]
			});
			for (let a = 1; a < i; a++) n.push({
				op: "lineTo",
				data: [r[a][0] + L(e, t), r[a][1] + L(e, t)]
			});
		}
	}
	return {
	...
	return function(e, t) {
		...
		if (!d[n]) switch (n) {
			...
			case "zigzag-line":
				d[n] || (d[n] = new u(t));
				break;
			default: n = "hachure", d[n] || (d[n] = new i(t));
		}
		return d[n];
	}(t, C).fillPolygons(e, t);
}
function P(e) {
...
function F(e) {
	return e.randomizer || (e.randomizer = new f(e.seed || 0)), e.randomizer.next();
}
function I(e, t, n, r = 1) {
	return n.roughness * r * (F(n) * (t - e) + e);
}
function L(e, t, n = 1) {
	return I(-e, e, t, n);
}
function R(e, t, n, r, i, a = !1) {
	let o = a ? i.disableMultiStrokeFill : i.disableMultiStroke, s = z(e, t, n, r, i, !0, !1);
	if (o) return s;
	let c = z(e, t, n, r, i, !0, !0);
	return s.concat(c);
}
function z(e, t, n, r, i, a, o) {
	...
	u * u * 100 > s && (u = c / 10);
	let d = u / 2, f = .2 + .2 * F(i), p = i.bowing * i.maxRandomnessOffset * (r - t) / 200, m = i.bowing * i.maxRandomnessOffset * (e - n) / 200;
	p = L(p, i, l), m = L(m, i, l);
	let h = [], g = () => L(d, i, l), _ = () => L(u, i, l), v = i.preserveVertices;
	return a && (o ? h.push({
		op: "move",
		data: [e + (v ? 0 : g()), t + (v ? 0 : g())]
	}) : h.push({
		op: "move",
		data: [e + (v ? 0 : L(u, i, l)), t + (v ? 0 : L(u, i, l))]
	})), o ? h.push({
		op: "bcurveTo",
		data: [
			p + e + (n - e) * f + g(),
...
function B(e, t, n) {
	if (!e.length) return [];
	let r = [];
	r.push([e[0][0] + L(t, n), e[0][1] + L(t, n)]), r.push([e[0][0] + L(t, n), e[0][1] + L(t, n)]);
	for (let i = 1; i < e.length; i++) r.push([e[i][0] + L(t, n), e[i][1] + L(t, n)]), i === e.length - 1 && r.push([e[i][0] + L(t, n), e[i][1] + L(t, n)]);
	return V(r, null, n);
}
function V(e, t, n) {
	let r = e.length, i = [];
	...
	})) : r === 2 && i.push(...z(e[0][0], e[0][1], e[1][0], e[1][1], n, !0, !0));
	return i;
}
function H(e, t, n, r, i, a, o, s) {
	...
	if (s.roughness === 0) {
		...
		let u = L(.5, s) - Math.PI / 2;
		l.push([L(a, s) + t + .9 * r * Math.cos(u - e), L(a, s) + n + .9 * i * Math.sin(u - e)]);
		let d = 2 * Math.PI + u - .01;
		for (let o = u; o < d; o += e) {
			let e = [L(a, s) + t + r * Math.cos(o), L(a, s) + n + i * Math.sin(o)];
			c.push(e), l.push(e);
		}
		l.push([L(a, s) + t + r * Math.cos(u + 2 * Math.PI + .5 * o), L(a, s) + n + i * Math.sin(u + 2 * Math.PI + .5 * o)]), l.push([L(a, s) + t + .98 * r * Math.cos(u + o), L(a, s) + n + .98 * i * Math.sin(u + o)]), l.push([L(a, s) + t + .9 * r * Math.cos(u + .5 * o), L(a, s) + n + .9 * i * Math.sin(u + .5 * o)]);
	}
	return [l, c];
}
function U(e, t, n, r, i, a, o, s, c) {
	let l = a + L(.1, c), u = [];
	u.push([L(s, c) + t + .9 * r * Math.cos(l - e), L(s, c) + n + .9 * i * Math.sin(l - e)]);
	for (let a = l; a <= o; a += e) u.push([L(s, c) + t + r * Math.cos(a), L(s, c) + n + i * Math.sin(a)]);
	return u.push([t + r * Math.cos(o), n + i * Math.sin(o)]), u.push([t + r * Math.cos(o), n + i * Math.sin(o)]), V(u, null, c);
}
function ee(e, t, n, r, i, a, o, s) {
	...
	for (let p = 0; p < d; p++) p === 0 ? c.push({
		...
		op: "move",
		data: [o[0] + (f ? 0 : L(l[0], s)), o[1] + (f ? 0 : L(l[0], s))]
	}), u = f ? [i, a] : [i + L(l[p], s), a + L(l[p], s)], c.push({
		op: "bcurveTo",
		data: [
			e + L(l[p], s),
			t + L(l[p], s),
...
function G(e, t = 0) {
	...
	let r = [];
	if (n === 3) r.push(W(e[0]), W(e[1]), W(e[2]), W(e[2]));
	else {
		let n = [];
		n.push(e[0], e[0]);
...
function te(e, t, n) {
	...
	return i = Math.max(0, Math.min(1, i)), K(e, q(t, n, i));
}
function q(e, t, n) {
...
function J(e, t, n, r) {
	...
	if (function(e, t) {
		...
		let n = e[t + 0];
		i.length ? (a = i[i.length - 1], o = n, Math.sqrt(K(a, o)) > 1 && i.push(n)) : i.push(n), i.push(e[t + 3]);
	} else {
		let r = .5, a = e[t + 0], o = e[t + 1], s = e[t + 2], c = e[t + 3], l = q(a, o, r), u = q(o, s, r), d = q(s, c, r), f = q(l, u, r), p = q(u, d, r), m = q(f, p, r);
		J([
			a,
			l,
			f,
...
function Y(e, t, n, r, i) {
	...
	return Math.sqrt(c) > r ? (Y(e, t, l + 1, r, a), Y(e, l, n, r, a)) : (a.length || a.push(o), a.push(s)), a;
}
function X(e, t = .15, n) {
	let r = [], i = (e.length - 1) / 3;
	for (let n = 0; n < i; n++) J(e, 3 * n, t, r);
...
var Z = "none", Q = class {
	...
	line(e, t, n, r, i) {
		let a = this._o(i);
		return this._d("line", [w(e, t, n, r, a)], a);
	}
	rectangle(e, t, n, r, i) {
		let a = this._o(i), o = [], s = E(e, t, n, r, a);
		if (a.fill) {
			let i = [
				[e, t],
				[e + n, t],
			...
			a.fillStyle === "solid" ? o.push(M([i], a)) : o.push(N([i], a));
		}
		return a.stroke !== Z && o.push(s), this._d("rectangle", o, a);
	}
	ellipse(e, t, n, r, i) {
		let a = this._o(i), o = [], s = O(n, r, a), c = k(e, t, a, s);
		if (a.fill) {
			if (a.fillStyle === "solid") {
				let n = k(e, t, a, s).opset;
				n.type = "fillPath", o.push(n);
			} else o.push(N([c.estimatedPoints], a));
		}
	...
	arc(e, t, n, r, i, a, o = !1, s) {
		...
		if (o && c.fill) {
			if (c.fillStyle === "solid") {
				...
				return m.push([s + l * Math.cos(f), c + u * Math.sin(f)]), m.push([s, c]), N([m], o);
			}(e, t, n, r, i, a, c));
		}
		return c.stroke !== Z && l.push(u), this._d("arc", l, c);
	}
	curve(e, t) {
		let n = this._o(t), r = [], i = D(e, n);
		if (n.fill && n.fill !== Z) {
			if (n.fillStyle === "solid") {
				let t = D(e, Object.assign(Object.assign({}, n), {
				...
				if (i.length) {
					...
					for (let r of e) r.length < 3 ? t.push(...r) : r.length === 3 ? t.push(...X(G([
						r[0],
						r[0],
						r[1],
						r[2]
					]), 10, (1 + n.roughness) / 2)) : t.push(...X(G(r), 10, (1 + n.roughness) / 2));
				}
				t.length && r.push(N([t], n));
			}
		}
		return n.stroke !== Z && r.push(i), this._d("curve", r, n);
	}
	polygon(e, t) {
		let n = this._o(t), r = [], i = T(e, !0, n);
		return n.fill && (n.fillStyle === "solid" ? r.push(M([e], n)) : r.push(N([e], n))), n.stroke !== Z && r.push(i), this._d("polygon", r, n);
	}
	path(e, t) {
		let n = this._o(t), r = [];
		if (!e) return this._d("path", r, n);
		e = (e || "").replace(/\n/g, " ").replace(/(-\s)/g, "-").replace("/(ss)/g", " ");
		let i = n.fill && n.fill !== "transparent" && n.fill !== Z, a = n.stroke !== Z, o = !!(n.simplification && n.simplification < 1), s = function(e, t, n) {
			let r = b(y(v(e))), i = [], a = [], o = [0, 0], s = [], c = () => {
			...
			return u;
		}(e, 1, o ? 4 - 4 * (n.simplification || 1) : (1 + n.roughness) / 2), c = j(e, n);
		if (i) {
			if (n.fillStyle === "solid") {
				if (s.length === 1) {
					let t = j(e, Object.assign(Object.assign({}, n), {
						disableMultiStroke: !0,
						roughness: n.roughness ? n.roughness + n.fillShapeRoughnessGain : 0
					}));
					r.push({
	...
	constructor(e, t) {
		this.canvas = e, this.ctx = this.canvas.getContext("2d"), this.gen = new Q(t);
	}
	draw(e) {
	...
	draw(e) {
		let t = e.sets || [], n = e.options || this.getDefaultOptions(), r = this.svg.ownerDocument || window.document, i = r.createElementNS($, "g"), a = e.options.fixedDecimalPlaceDigits;
		for (let o of t) {
			let t = null;
			switch (o.type) {
				case "path":
					...
					break;
				case "fillPath":
					t = r.createElementNS($, "path"), t.setAttribute("d", this.opsToPath(o, a)), t.setAttribute("stroke", "none"), t.setAttribute("stroke-width", "0"), t.setAttribute("fill", n.fill || ""), e.shape !== "curve" && e.shape !== "polygon" || t.setAttribute("fill-rule", "evenodd");
					break;
